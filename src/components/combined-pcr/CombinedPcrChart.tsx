...
export default function CombinedPcrChart({ reading, history }: CombinedPcrChartProps) {
  const series = useMemo(() => {
    const combined =
      history?.combined ?? (reading.combinedScore != null ? [reading.combinedScore] : []);
    ...
    >
      ...
      {[100, 60, 20, 0, -20, -60, -100].map((v) => (
        <g key={v}>
          ...
          />
          <text x={4} y={yLine(v) + 3} fontSize={10} fill="rgba(255,255,255,0.5)">
            {v}
          </text>
        </g>
      ))}
      ...
      {reading.combinedScore != null && (
        <g>
          <circle
            cx={W - PAD_X}
            cy={yLine(reading.combinedScore)}
            r={4}
            fill={
              reading.direction === "PE"
                ? "#22c55e"
                : reading.direction === "CE"
                  ? "#ef4444"
                  : "#9ca3af"
            }
          />
