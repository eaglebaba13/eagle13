...
describe("Phase 24D · DashboardCardHeader", () => {
  it("renders title and methodology badge", () => {
    const html = renderToStaticMarkup(
      <DashboardCardHeader title="Gold Silver" methodology="GOLD_SILVER_RATIO_V1" />,
    );
  ...
  it("renders freshness pill when provided", () => {
    const html = renderToStaticMarkup(
      <DashboardCardHeader title="X" freshness={fresh} provider="Yahoo" />,
    );
  ...
  it("collapse button exposes aria-expanded", () => {
    const html = renderToStaticMarkup(
      <DashboardCardHeader title="X" collapsed={false} onToggleCollapse={() => {}} />,
    );
  ...
  it("locked state renders lock icon with reason", () => {
    const html = renderToStaticMarkup(
      <DashboardCardHeader title="X" locked lockedReason="Pro plan required" />,
    );
