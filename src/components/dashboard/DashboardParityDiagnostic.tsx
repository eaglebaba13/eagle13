...
import { resolveDesktopNav, resolveMobileDrawerNav, type NavContext } from "@/lib/navigation";
  ...
  return (
    ...
    >
      ...
      <ul style={{ paddingLeft: 18, margin: 0, display: "grid", gap: 4 }}>
        <li>
          Registered widgets: <b>{audit.registered}</b>
        </li>
        <li>Duplicate IDs: {audit.duplicateIds.length ? audit.duplicateIds.join(", ") : "none"}</li>
        <li>
          Missing loaders: {audit.missingLoaders.length ? audit.missingLoaders.join(", ") : "none"}
        </li>
        ...
        <li>
          Desktop rendered: {desktop.length} · Mobile rendered: {mobile.length} · parity:{" "}
          <b>{parityOk ? "OK" : "MISMATCH"}</b>
        </li>
        <li>
          Nav parity: <b>{navParityOk ? "OK" : "MISMATCH"}</b> (desktop={navDesk.length}, mobile=
          {navMob.length})
        </li>
