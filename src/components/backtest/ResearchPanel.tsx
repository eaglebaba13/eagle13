...
const RecommendationValidationSection = lazy(() => import("./RecommendationValidationSection"));
...
import { buildComparisonMatrixCsv, buildResearchJson } from "@/lib/backtest/research-exports";
...
const STRATEGY_OPTIONS: StrategyOption[] = [
  {
    ...
    label: "Sign-Degree Astro",
    supported: true,
  },
  {
    id: "ASTRO",
    formula: ASTRO_FORMULA_VERSIONS.LEGACY_EAGLEBABA_CASCADE_V1,
    label: "Legacy Cascade",
    supported: true,
  },
  {
    id: "ASTRO",
    formula: "GANN_ASTRO_INTRADAY_ABSOLUTE_V1",
    label: "Absolute-Degree Intraday",
    supported: false,
    note: "Requires 5m provider payload · enable via CSV/provider selection",
  },
  {
    id: "SMC",
    formula: "SMC_V1",
...
function shimResult(src: BacktestResult, formula: UnifiedFormulaId): HistoricalBacktestResult {
  const trades = src.trades.filter((t) => t.signal !== "WAIT").map((t) => shimTrade(t, formula));
...
function sliceResult(
  ...
  const trades = src.trades
    .filter((t) => t.date >= from && t.date <= to && t.signal !== "WAIT")
  ...
  return {
    ...
    stats: {},
    monthly: [],
    equityCurve: src.equityCurve
      .filter((p) => p.date >= from && p.date <= to)
      .map((p) => ({ date: p.date, equity: p.cumulative })),
...
export default function ResearchPanel() {
  ...
  const [sortKey, setSortKey] = useState<"stability" | "vpf" | "vexp" | "dd" | "trades">(
    "stability",
  );
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (selected.length === 0)
        throw new Error("INSUFFICIENT_DATA · select at least one supported strategy.");
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        const walk = await runWalkForward(
          ...
          async (win, _phase, wi) => {
            setProgress(
              (p) =>
                p && { ...p, windowIndex: wi + 1, windowTotal: Math.max(p.windowTotal, wi + 1) },
            );
  ...
  const summary = useMemo(
    () => (comparison ? generateResearchSummary(comparison) : null),
    [comparison],
  );
    ...
    const cmp: Record<typeof sortKey, (a: StrategyResearchRow, b: StrategyResearchRow) => number> =
      {
        ...
        vpf: (a, b) =>
          (Number.isFinite(b.validation.profitFactor) ? b.validation.profitFactor : 1e9) -
          (Number.isFinite(a.validation.profitFactor) ? a.validation.profitFactor : 1e9),
  ...
  const exportMatrixCsv = () => {
    ...
    downloadBlob(
      header + csv,
      `research-matrix-${cfg.symbol}-${cfg.from}-${cfg.to}.csv`,
      "text/csv",
    );
  };
  const exportJson = () => {
  ...
  const exportWindowsCsv = () => {
    const lines: string[] = [
      "strategy,windowIndex,trainingFrom,trainingTo,validationFrom,validationTo,trainingTrades,validationTrades,trainingWR,validationWR,trainingPF,validationPF,trainingExpectancy,validationExpectancy,trainingDD,validationDD,pfDegradation,wrDegradation",
    ];
      ...
      for (const w of walk.windows) {
        lines.push(
          [
            ...
            Number.isFinite(w.trainingMetrics.profitFactor)
              ? w.trainingMetrics.profitFactor
              : "Infinity",
            Number.isFinite(w.validationMetrics.profitFactor)
              ? w.validationMetrics.profitFactor
              : "Infinity",
    ...
    downloadBlob(
      `# Research Run ${runId}\n` + lines.join("\n"),
      `research-windows-${cfg.from}-${cfg.to}.csv`,
      "text/csv",
    );
  };
  ...
  return (
    <div>
      <section style={panel}>
        ...
        <div
          style={{ fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted, marginBottom: 12 }}
        >
          RESEARCH ANALYSIS — NOT A LIVE TRADE RECOMMENDATION
        </div>
        ...
        >
          ...
          <div>
            ...
            <select
              value={cfg.splitMode}
              onChange={(e) => setCfg({ ...cfg, splitMode: e.target.value as SplitMode })}
              style={sel}
            >
              {SPLIT_MODES.map((m) => (
            ...
            <>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCfg({ ...cfg, windowDays: Number(e.target.value) || 30 })}
                  style={sel}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min={1}
                  value={cfg.stepDays}
                  onChange={(e) => setCfg({ ...cfg, stepDays: Number(e.target.value) || 10 })}
                  style={sel}
                />
              </div>
            </>
          ...
          <div>
            ...
            <select
              value={cfg.symbol}
              onChange={(e) => setCfg({ ...cfg, symbol: e.target.value as BacktestSymbol })}
              style={sel}
            >
          ...
          <div>
            ...
            <input
              ...
              min={5}
              value={cfg.minTrades}
              onChange={(e) => setCfg({ ...cfg, minTrades: Number(e.target.value) || 20 })}
              style={sel}
            />
          </div>
        ...
        {progress ? (
          <div
            style={{ marginTop: 10, fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted }}
          >
            Strategy {progress.strategyIndex + 1}/{progress.strategyTotal} · {progress.strategy} ·
            window {progress.windowIndex}/{progress.windowTotal} · {cfg.symbol} · {cfg.from} →{" "}
            {cfg.to}
          </div>
        ) : null}
        {error ? (
      ...
      {!rows && !running && !error ? (
        ...
        >
          Select strategies and research settings, then run the Research Lab to validate
          ...
          {tab === "wf" ? (
            <>
              ...
              <section style={panel}>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    ...
                    <button onClick={exportMatrixCsv} style={btnGhost}>
                      Matrix CSV
                    </button>
                    <button onClick={exportJson} style={btnGhost}>
                      Research JSON
                    </button>
                    <button onClick={exportWindowsCsv} style={btnGhost}>
                      Windows CSV
                    </button>
                  </div>
                </div>
                <div style={{ overflowX: "auto" }}>
                  ...
                  >
                    <thead>
                      <tr style={{ color: C.muted, textAlign: "left" }}>
                        {[
                          ...
                          <th
                            key={h}
                            style={{ padding: "6px 8px", borderBottom: `1px solid ${C.border}` }}
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {sortedRows.map((r, i) => (
                        <tr
                          key={`${r.strategy}-${r.formula}-${i}`}
                          style={{
                            borderBottom: `1px solid ${C.border}`,
                            opacity: r.status === "INSUFFICIENT_DATA" ? 0.55 : 1,
                          }}
                        >
                          ...
                          <td
                            style={{
                              padding: "6px 8px",
                              color: colorForPct(r.degradation.profitFactor),
                            }}
                          >
                            {fmt(r.degradation.profitFactor)}%
                          </td>
                          ...
                          <td
                            style={{
                              padding: "6px 8px",
                              color: colorForPct(r.degradation.winRate),
                            }}
                          >
                            {fmt(r.degradation.winRate)}%
                          </td>
                          ...
                          <td
                            style={{
                              padding: "6px 8px",
                              color:
                                r.stability.score >= 65
                                  ? C.green
                                  : r.stability.score >= 50
                                    ? C.orange
                                    : C.red,
                            }}
                          >
              ...
              <section style={panel}>
                ...
                >
                  {rows.map((r, i) => (
                    <div
                      key={`${r.strategy}-${i}`}
                      style={{ border: `1px solid ${C.border}`, borderRadius: 6, padding: 10 }}
                    >
                      <div
                        style={{
                          fontFamily: "var(--eb-mono)",
                      ...
                      >
                        Score: <span style={{ color: C.orange }}>{r.stability.score}</span> ·
                        Status: {r.status}
                      </div>
                        ...
                        <tbody>
                          {r.stability.factors.map((f) => (
                            <tr key={f.id}>
                              ...
                              <td
                                style={{ padding: "3px 4px", textAlign: "right", color: C.muted }}
                              >
                                ×{f.weight}
                              </td>
                            </tr>
              ...
              <section style={panel}>
                ...
                <div style={{ overflowX: "auto" }}>
                  ...
                  >
                    ...
                    <tbody>
                      {rows.map((r, i) => {
                        const cls = classifyDegradation(
                          r.degradation.netPnl,
                          r.validation.tradeCount,
                        );
                        return (
                          <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                            ...
                            <td
                              style={{
                                padding: "6px 8px",
                                color:
                                  cls === "STABLE" ? C.green : cls === "SEVERE" ? C.red : C.orange,
                              }}
                            >
                              {cls}
                            </td>
              ...
              <section style={panel}>
                ...
                >
                  <div>
                    Most stable: <span style={{ color: C.green }}>{summary.mostStable ?? "—"}</span>
                  </div>
                  <div>
                    Least stable: <span style={{ color: C.red }}>{summary.leastStable ?? "—"}</span>
                  </div>
              ...
              <section style={panel}>
                ...
                >
                  <div>
                    Research Run ID: <span style={{ color: C.blue }}>{runId}</span>
                  </div>
                  ...
                  <ul style={{ margin: 0, paddingLeft: 18 }}>
                    {Object.entries(walkByStrategy).map(([k, w]) => (
                      <li key={k}>
                        {k} · {w.windows[0]?.training.runId ?? "n/a"}
                      </li>
                    ))}
                  </ul>
...
function WalkWindowsTable({ label, walk }: { label: string; walk: WalkForwardResult }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontFamily: "var(--eb-mono)", fontSize: 12, color: C.text, marginBottom: 6 }}>
        {label} · {total} window{total === 1 ? "" : "s"}
      </div>
      <div style={{ overflowX: "auto" }}>
        <table
          style={{
            ...
            {view.map((w) => (
              <tr key={w.window.index} style={{ borderBottom: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: "4px 6px" }}>
                  {w.window.training.from} → {w.window.training.to}
                </td>
                <td style={{ padding: "4px 6px" }}>
                  {w.window.validation.from} → {w.window.validation.to}
                </td>
                ...
                <td style={{ padding: "4px 6px", color: colorForPct(w.degradation.profitFactor) }}>
                  {fmt(w.degradation.profitFactor)}%
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {pages > 1 ? (
        <div style={{ display: "flex", gap: 6, marginTop: 6, alignItems: "center" }}>
          ...
          <span style={{ fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted }}>
            Page {page + 1} / {pages}
          </span>
          <button
            style={btnGhost}
...
export function MonteCarloSection({
  ...
  const exportMcCsv = () => {
    if (mcResult)
      downloadBlob(
        buildMonteCarloCsv(mcResult, provenance),
        `monte-carlo-${instrument}-${from}-${to}.csv`,
        "text/csv",
      );
  };
  const exportMcJson = () => {
    if (mcResult)
      downloadBlob(
        buildMonteCarloJson(mcResult, provenance),
        `monte-carlo-${instrument}-${from}-${to}.json`,
        "application/json",
      );
  };
  const exportRobCsv = () => {
    if (robustness)
      downloadBlob(
        buildRobustnessCsv(robustness, provenance),
        `robustness-${instrument}-${from}-${to}.csv`,
        "text/csv",
      );
  };
  const exportRobJson = () => {
    if (robustness)
      downloadBlob(
        buildRobustnessJson(robustness, provenance),
        `robustness-${instrument}-${from}-${to}.json`,
        "application/json",
      );
  };

  const statusColor = !robustness
    ? C.muted
    : robustness.status === "ROBUST"
      ? C.green
      : robustness.status === "ACCEPTABLE"
        ? C.blue
        : robustness.status === "OVERFIT"
          ? C.orange
    ...
    <>
      {tab === "mc" ? (
        <section style={panel}>
          ...
          <div
            style={{ fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted, marginBottom: 10 }}
          >
            Resamples validation trades from the selected strategy. Deterministic for a given seed.
            RESEARCH ANALYSIS — NOT A LIVE TRADE RECOMMENDATION.
          </div>
          <div
            style={{
          ...
          >
            ...
            <div>
              ...
              <select
                value={mcCfg.samplingMode}
                onChange={(e) =>
                  setMcCfg({ ...mcCfg, samplingMode: e.target.value as MonteCarloSamplingMode })
                }
                style={sel}
              >
            ...
            <div>
              ...
              <input
                type="number"
                min={10}
                max={MONTE_CARLO_UI_MAX_SIMULATIONS}
                value={mcCfg.simulations}
                onChange={(e) => setMcCfg({ ...mcCfg, simulations: Number(e.target.value) || 500 })}
                style={sel}
              />
            </div>
            <div>
              ...
              <input
                type="number"
                value={mcCfg.seed}
                onChange={(e) => setMcCfg({ ...mcCfg, seed: Number(e.target.value) || 0 })}
                style={sel}
              />
            </div>
            <div>
              ...
              <input
                type="number"
                min={1}
                value={mcCfg.startingCapital}
                onChange={(e) =>
                  setMcCfg({ ...mcCfg, startingCapital: Number(e.target.value) || 100000 })
                }
                style={sel}
              />
            </div>
            <div>
              ...
              <select
                value={mcCfg.ruinDrawdownPct}
                onChange={(e) => setMcCfg({ ...mcCfg, ruinDrawdownPct: Number(e.target.value) })}
                style={sel}
              >
            ...
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              <button
                ...
                style={{ ...btn, opacity: running || trades.length === 0 ? 0.55 : 1 }}
              >
          ...
          {trades.length === 0 ? (
            <div
              style={{ marginTop: 8, color: C.muted, fontFamily: "var(--eb-mono)", fontSize: 11 }}
            >
              No validation trades available for the selected strategy.
            </div>
          ) : null}
          {error ? (
            <div style={{ marginTop: 8, color: C.red, fontFamily: "var(--eb-mono)", fontSize: 12 }}>
              {error}
            </div>
          ) : null}
            ...
            <div style={{ marginTop: 12 }}>
              ...
              >
                <McCard
                  ...
                  value={`${(mcResult.probabilityOfLoss * 100).toFixed(1)}%`}
                  accent={mcResult.probabilityOfLoss > 0.4 ? C.red : C.green}
                />
                <McCard
                  label="Probability of ruin"
                  value={`${(mcResult.probabilityOfRuin * 100).toFixed(1)}%`}
                  accent={mcResult.probabilityOfRuin > 0.1 ? C.red : C.green}
                />
                ...
                <McCard
                  label="Max DD · P95"
                  value={mcResult.maxDrawdown.p95.toFixed(0)}
                  accent={C.red}
                />
              </div>
              <div
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap" }}>
                <button onClick={exportMcCsv} style={btnGhost}>
                  Monte Carlo CSV
                </button>
                <button onClick={exportMcJson} style={btnGhost}>
                  Monte Carlo JSON
                </button>
              </div>
              <div
      ...
      {tab === "rob" && robustness ? (
        <section style={panel}>
          ...
          <div
            style={{ fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted, marginBottom: 8 }}
          >
            {robustness.reason}
          </div>
          {robustness.factors.length ? (
            <div style={{ overflowX: "auto" }}>
              ...
              >
                <thead>
                  <tr style={{ color: C.muted, textAlign: "left" }}>
                    {["Factor", "Weight", "Value", "Score", "Formula"].map((h) => (
                      <th
                        key={h}
                        style={{ padding: "4px 6px", borderBottom: `1px solid ${C.border}` }}
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {robustness.factors.map((f) => (
                    <tr key={f.key} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td
                        style={{
                          padding: "4px 6px",
                          color: f.score >= 0.7 ? C.green : f.score >= 0.4 ? C.orange : C.red,
                        }}
                      >
                        {f.score.toFixed(2)}
                      </td>
          ...
          <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button onClick={exportRobCsv} style={btnGhost}>
              Robustness CSV
            </button>
            <button onClick={exportRobJson} style={btnGhost}>
              Robustness JSON
            </button>
          </div>
      ...
      {tab === "rob" && !robustness ? (
        <section style={panel}>
          ...
          <div style={{ fontFamily: "var(--eb-mono)", fontSize: 12, color: C.muted }}>
            Run Monte Carlo first to compute the composite robustness score for the selected
            strategy.
          </div>
        </section>
      ) : null}
      ...
      {tab === "regime" ? (
        <Suspense
          fallback={<div style={{ padding: 12, opacity: 0.6 }}>Loading regime intelligence…</div>}
        >
      ...
      {tab === "recval" ? (
        <Suspense
          fallback={
            <div style={{ padding: 12, opacity: 0.6 }}>Loading recommendation validation…</div>
          }
        >
      ...
      {tab === "shadow" ? (
        <Suspense
          fallback={<div style={{ padding: 12, opacity: 0.6 }}>Loading shadow validation…</div>}
        >
          <ShadowSection />
        </Suspense>
      ) : null}
      {tab === "decision" ? (
        <Suspense
          fallback={<div style={{ padding: 12, opacity: 0.6 }}>Loading decision center…</div>}
        >
...
function SensitivitySection({ instrument }: { instrument: string }) {
  ...
  const [axisA, setAxisA] = useState<SensitivityAxisState>({
    name: "minScore",
    min: 40,
    max: 80,
    step: 10,
  });
  const [axisB, setAxisB] = useState<SensitivityAxisState>({
    name: "rr",
    min: 1,
    max: 3,
    step: 0.5,
  ...
  const [metric, setMetric] = useState<
    "profitFactor" | "expectancy" | "netPnl" | "maxDrawdown" | "stabilityScore"
  >("expectancy");
  ...
  const specs: ParameterSpec[] = useMemo(() => {
    const list: ParameterSpec[] = [
      { name: axisA.name, min: axisA.min, max: axisA.max, step: axisA.step },
    ];
    if (mode === "2D")
      list.push({ name: axisB.name, min: axisB.min, max: axisB.max, step: axisB.step });
    return list;
  }, [axisA, axisB, mode]);
  ...
  const runNow = useCallback(async () => {
    ...
    try {
      ...
      const surface = classifySensitivitySurface(
        result.cells,
        metric === "stabilityScore" ? "stabilityScore" : metric,
      );
  ...
  const classifiedSurface = useMemo(() => {
    ...
    return classifySensitivitySurface(
      outcome.cells,
      metric === "stabilityScore" ? "stabilityScore" : metric,
    );
  }, [outcome, metric]);
  ...
  const chartSeries = useMemo(() => {
    ...
    return sortedY.map((yv) => ({
      ...
      data: outcome.cells
        ...
        .map((c) => ({
          x: String(c.params[ax.name]),
          y: c.metrics ? Number(c.metrics[metric]) : 0,
        })),
    }));
  }, [outcome, metric]);
  ...
  const exportCells = () => {
    ...
    downloadBlob(
      csv,
      `sensitivity-cells-${instrument}-${payload.actualRange.from}-${payload.actualRange.to}.csv`,
      "text/csv",
    );
  };
  const exportMatrix = () => {
    if (!outcome || !payload) return;
    const prov = buildProvenance(outcome, payload);
    const csv = buildSensitivityMatrixCsv(
      outcome.cells,
      prov,
      metric === "stabilityScore" ? "expectancy" : metric,
    );
    downloadBlob(
      csv,
      `sensitivity-matrix-${instrument}-${payload.actualRange.from}-${payload.actualRange.to}.csv`,
      "text/csv",
    );
  };
  const exportJson = () => {
    ...
    downloadBlob(
      json,
      `sensitivity-${instrument}-${payload.actualRange.from}-${payload.actualRange.to}.json`,
      "application/json",
    );
  };
  const exportBundle = () => {
    ...
    downloadBlob(
      bundle,
      `research-bundle-${instrument}-${payload.actualRange.from}-${payload.actualRange.to}.json`,
      "application/json",
    );
  };
    ...
    <section style={panel}>
      ...
      {!payload ? (
        <div
          ...
          aria-live="polite"
        >
          {SENSITIVITY_UI_ERROR_LABEL.RESEARCH_PAYLOAD_MISSING} — RESEARCH_PAYLOAD_MISSING. Open the
          SMC or Hybrid Backtest panel, run it once with valid data, and return here.
        </div>
      ) : (
      ...
      >
        <div>
          ...
          <select
            value={strategy}
            onChange={(e) => setStrategy(e.target.value as SensitivityStrategy)}
            style={sel}
          >
        ...
        <div>
          ...
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as SensitivityMode)}
            style={sel}
          >
      ...
      >
        <div>
          ...
          <div style={{ color: cells > RESEARCH_UI_MAX_CELLS ? C.red : C.text }}>
            {cells} / {RESEARCH_UI_MAX_CELLS}
          </div>
        </div>
        <div>
          <div style={lbl}>Grid Status</div>
          <div style={{ color: validation.ok ? C.green : C.red }}>
            {validation.ok ? "OK" : validation.code}
          </div>
        </div>
        <div>
          <div style={lbl}>Effective A</div>
          <div style={{ color: C.muted }}>
            {axisA.name} · {axisA.min}→{axisA.max} step {axisA.step}
          </div>
        </div>
        {mode === "2D" ? (
          <div>
            ...
            <div style={{ color: C.muted }}>
              {axisB.name} · {axisB.min}→{axisB.max} step {axisB.step}
            </div>
          </div>
        ) : null}
      </div>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
        ...
        <button
          onClick={cancel}
          disabled={!running}
          style={{ ...btnGhost, opacity: running ? 1 : 0.5 }}
        >
          Cancel
        </button>
      </div>

      {progress ? <SensitivityProgress progress={progress} /> : null}
...
function PayloadSummary({ payload }: { payload: PublishedResearchPayload }) {
  ...
  return (
    ...
    >
      <div>
        <span style={{ color: C.muted }}>Instrument</span> {payload.instrument}
      </div>
      <div>
        <span style={{ color: C.muted }}>Timeframe</span> {String(payload.timeframe)}
      </div>
      <div>
        <span style={{ color: C.muted }}>Provider</span> {payload.provider}
      </div>
      <div>
        <span style={{ color: C.muted }}>Range</span> {payload.actualRange.from} →{" "}
        {payload.actualRange.to}
      </div>
      <div>
        <span style={{ color: C.muted }}>Candles</span> {payload.candles.length}
      </div>
      <div>
        <span style={{ color: C.muted }}>Data Hash</span> {payload.dataHash}
      </div>
      <div>
        <span style={{ color: C.muted }}>Data Quality</span>{" "}
        <span
          style={{
            color: bad ? C.red : payload.dataQuality.status === "DEGRADED" ? C.orange : C.green,
          }}
        >
      ...
      <div style={{ gridColumn: "1 / -1", color: C.muted, wordBreak: "break-all" }}>
        Base Run ID: <span style={{ color: C.blue }}>{payload.baseRunId}</span> · Strategy:{" "}
        {payload.strategy}
      </div>
    </div>
  );
...
function SensitivityProgress({
  ...
  return (
    <div style={{ marginTop: 10, fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted }}>
      Cell {progress.completed}/{progress.total} · {pct}% · elapsed {Math.round(elapsedMs / 1000)}s
      · eta {Math.round(remainingMs / 1000)}s
...
function SensitivityResults({
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      >
        <div>
          <span style={{ color: C.muted }}>Cells</span> {outcome.cells.length}
        </div>
        <div>
          <span style={{ color: C.muted }}>Valid</span> {valid.length}
        </div>
        <div>
          <span style={{ color: C.muted }}>Insufficient</span> {invalid}
        </div>
        <div>
          <span style={{ color: C.muted }}>Classification</span> {surface?.classification ?? "n/a"}
        </div>
        <div>
          <span style={{ color: C.muted }}>Partial</span> {outcome.partial ? "YES" : "no"}
        </div>
        <div>
          <span style={{ color: C.muted }}>Executions</span> {outcome.counters.executionCount ?? 0}
        </div>
        <div>
          <span style={{ color: C.muted }}>Provider fetches</span>{" "}
          <span
            style={{ color: (outcome.counters.providerLoadCount ?? 0) === 0 ? C.green : C.red }}
          >
      ...
      <ApexChart
        ...
        options={{
          ...
          xaxis: {
            ...
            title: { text: outcome.grid[0]?.name ?? "" },
          },
          yaxis: {
            labels: { style: { colors: "var(--eb-muted)" } },
            title: { text: chartType === "line" ? metric : (outcome.grid[1]?.name ?? "") },
          },
          ...
          <thead>
            <tr style={{ color: C.muted, textAlign: "left" }}>
              {[
                ...
                "Stability",
                "OOS",
                metric === "stabilityScore" ? "score" : metric,
                "status",
              ].map((h) => (
                <th key={h} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.border}` }}>
                  {h}
            ...
            {outcome.cells.map((c, i) => {
              ...
              return (
                <tr
                  ...
                  style={{
                    borderBottom: `1px solid ${C.border}`,
                    background: isSel ? "rgba(255,140,0,0.08)" : "transparent",
                    cursor: "pointer",
                    opacity: m ? 1 : 0.55,
                  }}
                >
                  {outcome.grid.map((g) => (
                    <td key={g.name} style={{ padding: "4px 6px" }}>
                      {c.params[g.name]}
                    </td>
                  ))}
                  ...
                  <td style={{ padding: "4px 6px" }}>
                    {m ? String(m[metric as keyof typeof m] ?? "—") : "—"}
                  </td>
                  <td style={{ padding: "4px 6px", color: m ? C.green : C.muted }}>
                    {m ? "VALID" : (c.reason ?? "INSUFFICIENT")}
                  </td>
                </tr>
              );
            })}
      ...
      {selected ? (
        <div
          style={{ marginTop: 10, border: `1px solid ${C.border}`, borderRadius: 6, padding: 10 }}
        >
          <div
            style={{ fontFamily: "var(--eb-mono)", fontSize: 12, color: C.orange, marginBottom: 6 }}
          >
            Cell Details
          </div>
          <div
          ...
          >
            ...
            {selected.metrics ? (
              <>
                <div>
                  <span style={{ color: C.muted }}>Trades</span> {selected.metrics.trades}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Win Rate</span>{" "}
                  {(selected.metrics.winRate * 100).toFixed(1)}%
                </div>
                <div>
                  <span style={{ color: C.muted }}>PF</span> {fmt(selected.metrics.profitFactor)}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Expectancy</span> {selected.metrics.expectancy}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Net PnL</span> {selected.metrics.netPnl}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Max DD</span> {selected.metrics.maxDrawdown}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Recovery</span>{" "}
                  {fmt(selected.metrics.recoveryFactor)}
                </div>
                <div>
                  <span style={{ color: C.muted }}>Stability</span>{" "}
                  {selected.metrics.stabilityScore}
                </div>
                <div>
                  <span style={{ color: C.muted }}>OOS</span> {selected.metrics.oosScore}
                </div>
                <div>
                  <span style={{ color: C.muted }}>MC p5</span> {selected.metrics.monteCarloP5}
                </div>
              </>
            ) : (
              <div style={{ gridColumn: "1 / -1", color: C.red }}>
                {selected.reason ?? "INSUFFICIENT_DATA"}
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap" }}>
        <button onClick={exportCells} style={btnGhost}>
          Cells CSV
        </button>
        <button onClick={exportMatrix} style={btnGhost}>
          Matrix CSV
        </button>
        <button onClick={exportJson} style={btnGhost}>
          Sensitivity JSON
        </button>
        <button onClick={exportBundle} style={btnGhost}>
      ...
      >
        Sensitivity Run ID: <span style={{ color: C.blue }}>{outcome.runId}</span>
...
function toCxRows(rows: StrategyResearchRow[], instrument: string): CrossAssetRow[] {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      recoveryFactor:
        r.validation.drawdown > 0
          ? (r.validation.expectancy * decided) / r.validation.drawdown
          : null,
    ...
    <>
          ...
          <div style={{ overflowX: "auto" }}>
            ...
            >
              ...
              <tbody>
                {cxRows.map((r, i) => (
                  <tr
                    key={`${r.instrument}-${r.strategy}-${i}`}
                    style={{
                      borderBottom: `1px solid ${C.border}`,
                      opacity: r.sufficient ? 1 : 0.55,
                    }}
                  >
          ...
          >
            {leaderboard.map((e) => (
              ...
              >
                ...
                <div style={{ fontFamily: "var(--eb-mono)", fontSize: 14, marginTop: 4 }}>
                  {e.winner ?? "—"}
                </div>
                <div
                  style={{
      ...
      {cxRows.length > 0 ? (
        <section style={panel}>
          ...
          >
            <div>
              <span style={lbl}>Best Instrument</span>
              <div>{summary.bestInstrument ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.bestInstrument}</div>
            </div>
            <div>
              <span style={lbl}>Weak Instrument</span>
              <div>{summary.weakInstrument ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.weakInstrument}</div>
            </div>
            <div>
              <span style={lbl}>Best Timeframe</span>
              <div>{summary.bestTimeframe ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.bestTimeframe}</div>
            </div>
            <div>
              <span style={lbl}>Weak Timeframe</span>
              <div>{summary.weakTimeframe ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.weakTimeframe}</div>
            </div>
            <div>
              <span style={lbl}>Best Regime</span>
              <div>{summary.bestRegime ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.bestRegime}</div>
            </div>
            <div>
              <span style={lbl}>Worst Regime</span>
              <div>{summary.worstRegime ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>{summary.reasons.worstRegime}</div>
            </div>
            <div>
              <span style={lbl}>Highest Confidence</span>
              <div>{summary.highestConfidenceStrategy ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>
                {summary.reasons.highestConfidenceStrategy}
              </div>
            </div>
            <div>
              <span style={lbl}>Least Stable</span>
              <div>{summary.leastStableStrategy ?? "—"}</div>
              <div style={{ color: C.muted, fontSize: 10 }}>
                {summary.reasons.leastStableStrategy}
              </div>
            </div>
          </div>
        </section>
      ...
      {cxRows.length > 0 ? (
        <section style={panel}>
            ...
            {strategies.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontFamily: "var(--eb-mono)", fontSize: 24, marginTop: 6 }}>
                    {c.score}
                  </div>
                  <div
                    style={{
          ...
          <div style={{ marginTop: 8, fontFamily: "var(--eb-mono)", fontSize: 10, color: C.muted }}>
            Matrix rows: {matrix.rowKeys.length} × {matrix.colKeys.length} · Regime rows:{" "}
            {regimeMatrix.rowKeys.length}
          </div>
        </section>
      ) : null}
    </>
...
function BatchSection() {
  ...
  const doExport = useCallback(
    (kind: "results-csv" | "results-json" | "failures-csv" | "coverage-csv" | "summary-json") => {
      if (!state) return;
      ...
      const map: Record<string, { name: string; mime: string; body: string }> = {
        "results-csv": {
          name: "batch-results.csv",
          mime: "text/csv",
          body: buildBatchResultsCsv(state, prov),
        },
        "results-json": {
          name: "batch-results.json",
          mime: "application/json",
          body: buildBatchResultsJson(state, prov),
        },
        "failures-csv": {
          name: "batch-failures.csv",
          mime: "text/csv",
          body: buildBatchFailuresCsv(state, prov),
        },
        "coverage-csv": {
          name: "batch-coverage.csv",
          mime: "text/csv",
          body: buildBatchCoverageCsv(state, prov),
        },
        "summary-json": {
          name: "batch-summary.json",
          mime: "application/json",
          body: buildBatchSummaryJson(state, prov),
        },
      };
      const spec = map[kind];
    ...
    <section style={panel}>
      ...
      >
        <div>
          ...
          {BATCH_STRATEGY_CHOICES.map((s) => (
            <label
              key={s}
              style={{ display: "block", fontFamily: "var(--eb-mono)", fontSize: 11, marginTop: 4 }}
            >
              <input
                type="checkbox"
                checked={!!selectedStrategies[s]}
                onChange={(e) => setSelectedStrategies((x) => ({ ...x, [s]: e.target.checked }))}
              />{" "}
              {s}
            </label>
          ))}
          ...
          {BATCH_INSTRUMENTS.map((i) => (
            ...
            >
              <input
                ...
                onChange={(e) => setSelectedInstruments((x) => ({ ...x, [i]: e.target.checked }))}
              />{" "}
              {i}
            </label>
          ))}
          ...
          {BATCH_TIMEFRAMES.map((t) => (
            ...
            >
              <input
                ...
                onChange={(e) => setSelectedTf((x) => ({ ...x, [t]: e.target.checked }))}
              />{" "}
              {t}
            </label>
          ))}
        ...
        <div>
          ...
          {BATCH_PERIODS.map((p) => (
            ...
            >
              <input
                type="checkbox"
                checked={!!selectedPeriods[p.label]}
                onChange={(e) => setSelectedPeriods((x) => ({ ...x, [p.label]: e.target.checked }))}
              />{" "}
              {p.label}
            </label>
          ))}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
        <button
          ...
          style={{ ...chip, background: running ? C.border : C.orange, color: "#04140b" }}
        >
        ...
        <button onClick={() => controllerRef.current?.cancel()} disabled={!running} style={chip}>
          Cancel
        </button>
        <button
          onClick={() => controllerRef.current?.restartFailed()}
        ...
        <button
          onClick={() => controllerRef.current?.restartAll()}
          disabled={running || !state}
          style={chip}
        >
          Restart All
      ...
      {state ? (
        <>
          ...
          >
            ...
            <McCard
              label="ETA"
              value={
                state.progress.etaMs === null ? "—" : `${Math.round(state.progress.etaMs / 1000)}s`
              }
            />
          </div>
          ...
          {state.progress.currentJobs.length > 0 ? (
            ...
            >
              Running:{" "}
              {state.progress.currentJobs
                .map((j) => `${j.strategy}/${j.instrument}/${j.timeframe}/${j.period.label}`)
                .join(", ")}
            </div>
          ) : null}
          ...
          <div style={{ overflowX: "auto", marginBottom: 10 }}>
            ...
            >
              ...
              <tbody>
                {state.records.map((r) => {
                  const pnl = r.result
                    ? Math.round(r.result.trades.reduce((s, t) => s + t.pnl, 0) * 100) / 100
                    : null;
                  const color =
                    r.status === "completed"
                      ? C.green
                  ...
                  return (
                    <tr key={r.key} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: "4px 6px", color: C.muted, fontSize: 10 }}>
                        {r.runId ?? r.error?.code ?? "—"}
                      </td>
                    </tr>
                  );
                })}
            ...
            >
              ...
              >
                <div>
                  ...
                  <div>{summary.coveragePct}%</div>
                </div>
                <div>
                  <span style={lbl}>Best Strategy</span>
                  <div>{summary.bestStrategy ?? "—"}</div>
                </div>
                <div>
                  <span style={lbl}>Best Instrument</span>
                  <div>{summary.bestInstrument ?? "—"}</div>
                ...
                <div>
                  <span style={lbl}>Best Period</span>
                  <div>{summary.bestPeriod ?? "—"}</div>
                </div>
                <div>
                  <span style={lbl}>Highest Net PnL</span>
                  <div>{summary.highestNetPnl === null ? "—" : summary.highestNetPnl}</div>
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button onClick={() => doExport("results-csv")} style={chip}>
              Results CSV
            </button>
            <button onClick={() => doExport("results-json")} style={chip}>
              Results JSON
            </button>
            <button onClick={() => doExport("failures-csv")} style={chip}>
              Failures CSV
            </button>
            <button onClick={() => doExport("coverage-csv")} style={chip}>
              Coverage CSV
            </button>
            <button onClick={() => doExport("summary-json")} style={chip}>
              Summary JSON
            </button>
          </div>
        </>
        ...
        >
          Configure your batch above and press Start. Runs use the shared unified backtest engine —
          no new runner.
        </div>
      )}
    </section>
