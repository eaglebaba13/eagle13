...
type FormShape = {
  ...
  rb_id: string;
  rb_score: string;
  rb_verdict: "ROBUST" | "MARGINAL" | "OVERFIT" | "UNRELIABLE";
  sn_id: string;
  sn_cliff: string;
  sn_plat: string;
  op_id: string;
  ...
  rv_id: string;
  rv_rel: string;
  rv_verdict: "RELIABLE" | "MARGINAL" | "UNRELIABLE";
  cx_id: string;
  cx_cons: string;
  cx_assets: string;
  pf_id: string;
  pf_rec: "ACCEPT" | "REVIEW" | "REJECT";
  pf_dd: string;
  pf_div: string;
  sh_id: string;
  sh_ready:
...
const DEFAULTS: FormShape = {
  ...
  pf_div: "",
  sh_id: "",
  sh_ready: "READY_FOR_SCHEDULED_SHADOW",
  sh_acc: "",
  sh_cal: "",
  sh_res: "",
  rc_id: "",
...
function toInput(f: FormShape): DecisionEvidenceInput {
  ...
  const wf_oos = n(f.wf_oos);
  const wf_stab = n(f.wf_stab);
  const wf_trades = n(f.wf_trades);
  if (f.wf_id && wf_oos !== null && wf_stab !== null && wf_trades !== null) {
    (inp as any).walkForward = {
      runId: f.wf_id,
      oosExpectancy: wf_oos,
      stabilityScore: wf_stab,
      overfitFlag: f.wf_overfit,
      totalTrades: wf_trades,
    };
  }
  const mc_dd = n(f.mc_dd);
  const mc_c = n(f.mc_cagr);
  const mc_r = n(f.mc_ruin);
  if (f.mc_id && mc_dd !== null && mc_c !== null && mc_r !== null) {
    (inp as any).monteCarlo = {
      runId: f.mc_id,
      worstDrawdownPct: mc_dd,
      medianCagr: mc_c,
      ruinProbability: mc_r,
    };
  }
  const rb_s = n(f.rb_score);
  if (f.rb_id && rb_s !== null)
    (inp as any).robustness = { runId: f.rb_id, score: rb_s, verdict: f.rb_verdict };
  const sn_c = n(f.sn_cliff);
  const sn_p = n(f.sn_plat);
  if (f.sn_id && sn_c !== null && sn_p !== null)
    (inp as any).sensitivity = { runId: f.sn_id, cliffScore: sn_c, plateauCoverage: sn_p };
  const op_c = n(f.op_conf);
  if (f.op_id && op_c !== null)
    (inp as any).optimizer = { runId: f.op_id, confidence: op_c, selectedCandidate: f.op_cand };
  const rv_r = n(f.rv_rel);
  if (f.rv_id && rv_r !== null)
    (inp as any).recommendationValidator = {
      runId: f.rv_id,
      reliability: rv_r,
      verdict: f.rv_verdict,
    };
  const cx_c = n(f.cx_cons);
  const cx_a = n(f.cx_assets);
  if (f.cx_id && cx_c !== null && cx_a !== null)
    (inp as any).crossAsset = { runId: f.cx_id, consistency: cx_c, assetsCovered: cx_a };
  const pf_d = n(f.pf_dd);
  const pf_v = n(f.pf_div);
  if (f.pf_id && pf_d !== null && pf_v !== null)
    (inp as any).portfolio = {
      runId: f.pf_id,
      recommendation: f.pf_rec,
      expectedDrawdown: pf_d,
      diversificationScore: pf_v,
    };
  const sh_a = n(f.sh_acc);
  const sh_c = n(f.sh_cal);
  const sh_r = n(f.sh_res);
  if (f.sh_id && sh_a !== null && sh_c !== null && sh_r !== null)
    (inp as any).shadow = {
      runId: f.sh_id,
      readiness: f.sh_ready,
      accuracy: sh_a,
      calibration: sh_c,
      resolvedTrades: sh_r,
    };
  const rc_w = n(f.rc_wr);
  const rc_p = n(f.rc_pf);
  const rc_c = n(f.rc_conf);
  if (f.rc_id && rc_w !== null && rc_p !== null && rc_c !== null)
    (inp as any).recommendation = {
      runId: f.rc_id,
      expectedWinRate: rc_w,
      expectedProfitFactor: rc_p,
      confidence: rc_c,
    };
  const rs_s = n(f.rs_stab);
  if (f.rs_id && rs_s !== null)
    (inp as any).researchStability = { runId: f.rs_id, stability: rs_s };
  const rg_c = n(f.rg_cov);
  if (f.rg_id && rg_c !== null) (inp as any).regime = { runId: f.rg_id, coverage: rg_c };
  if (f.dq_hash)
    (inp as any).dataQuality = { ok: f.dq_ok, causalityOk: f.dq_causal, dataHash: f.dq_hash };
  return inp;
}
...
function stateColor(s: string): string {
  ...
  if (
    s.startsWith("READY_FOR_MANUAL") ||
    s.startsWith("READY_FOR_SCHEDULED") ||
    s.startsWith("READY_FOR_PAPER")
  )
    return C.orange;
    ...
    <div>
      <section style={{ ...panel, borderColor: stateColor(result.state) }}>
        ...
        >
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontFamily: "var(--eb-mono)", fontSize: 20, color: C.text }}>
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap" }}>
          <button
            style={btn}
            onClick={() =>
              downloadBlob(`decision-${runId}.csv`, buildDecisionCsv(result, runId), "text/csv")
            }
          >
            Decision CSV
          </button>
          <button
            ...
            onClick={() =>
              downloadBlob(
                `decision-${runId}.json`,
                buildDecisionJson(result, runId),
                "application/json",
              )
            }
          >
          ...
          <button
            ...
            onClick={() =>
              downloadBlob(
                `decision-bundle-${runId}.json`,
                buildDecisionBundle(evidence, result),
                "application/json",
              )
            }
          >
        ...
        <div style={{ fontFamily: "var(--eb-mono)", fontSize: 10, color: C.muted, marginTop: 8 }}>
          Evaluates research evidence only. Does NOT generate trade signals or interact with any
          broker.
        </div>
      </section>
        ...
        >
          {result.checklist.map((it) => (
            ...
            >
              ...
              <div
                style={{ fontFamily: "var(--eb-mono)", fontSize: 10, color: C.muted, marginTop: 2 }}
              >
                {it.detail}
              </div>
            </div>
        ...
        >
          <thead>
            <tr style={{ color: C.muted, textAlign: "left" }}>
              {["Component", "Weight", "Score", "Present"].map((h) => (
          ...
          <tbody>
            {result.components.map((cc) => (
              <tr key={cc.key} style={{ borderBottom: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: "4px 6px", color: cc.present ? C.green : C.red }}>
                  {cc.present ? "YES" : "MISSING"}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <section style={panel}>
        <div style={{ ...label, marginBottom: 10 }}>
          Evidence Inputs (paste Run IDs and metrics from completed research tabs)
        </div>
        ...
        <Group title="Walk-Forward">
          <Field
            lbl="Run ID"
            value={f.wf_id}
            onChange={(v) => set("wf_id", v)}
            placeholder="WF:..."
          />
          <Field
            lbl="OOS Expectancy (R)"
            value={f.wf_oos}
            onChange={(v) => set("wf_oos", v)}
            placeholder="0.6"
          />
          <Field
            lbl="Stability Score"
            value={f.wf_stab}
            onChange={(v) => set("wf_stab", v)}
            placeholder="0..1"
          />
          <Field
            lbl="Total Trades"
            value={f.wf_trades}
            onChange={(v) => set("wf_trades", v)}
            placeholder="200"
          />
          <label
            style={{
          ...
          >
            <input
              type="checkbox"
              checked={f.wf_overfit}
              onChange={(e) => set("wf_overfit", e.target.checked)}
            />{" "}
            Overfit Flag
          </label>
        </Group>
        ...
        <Group title="Robustness">
          ...
          <Select
            lbl="Verdict"
            value={f.rb_verdict}
            onChange={(v) => set("rb_verdict", v as any)}
        ...
        <Group title="Recommendation Validator">
          ...
          <Select
            lbl="Verdict"
            value={f.rv_verdict}
            onChange={(v) => set("rv_verdict", v as any)}
        ...
        <Group title="Portfolio">
          ...
          <Select
            lbl="Recommendation"
            value={f.pf_rec}
            onChange={(v) => set("pf_rec", v as any)}
        ...
        <Group title="Shadow Validation">
          ...
          <Select
            lbl="Readiness"
            value={f.sh_ready}
            onChange={(v) => set("sh_ready", v as any)}
            options={[
              "READY_FOR_SCHEDULED_SHADOW",
              "READY_FOR_MANUAL_OBSERVATION",
              "PAUSED_BY_DATA_QUALITY",
        ...
        <Group title="Data Quality">
          ...
          >
            <input
              type="checkbox"
              checked={f.dq_ok}
              onChange={(e) => set("dq_ok", e.target.checked)}
            />{" "}
            Data OK
          </label>
          <label
          ...
          >
            <input
              type="checkbox"
              checked={f.dq_causal}
              onChange={(e) => set("dq_causal", e.target.checked)}
            />{" "}
            Causality OK
          </label>
        </Group>
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
          <button style={btn} onClick={() => setF(DEFAULTS)}>
            Reset
          </button>
        </div>
      </section>
