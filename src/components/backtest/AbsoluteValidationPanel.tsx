// Phase 21.3d-β2b · Shared Absolute-Degree Intraday validation panel.
// Extracted verbatim from /absolute-intraday-validation so both that
// dedicated route and the /backtest terminal can mount the same
// deterministic controls, sessions, exports and disclaimer without any
// engine, cache-key or Run-ID drift.
...
export type AbsoluteValidationPanelProps = {
  /** Optional test hook exposing run state without leaking network. */
  onResult?: (r: HistoryResult) => void;
};

export function AbsoluteValidationPanel({ onResult }: AbsoluteValidationPanelProps = {}) {
  ...
  // Memoize the session log rows so re-renders (filter changes on parent
  // page, resizes, etc.) do not re-map the whole result set every tick.
  const sessionRows = useMemo(() => result?.sessionsSummary ?? [], [result]);
  ...
  const onRun = async () => {
    // Explicit guard against overlapping runs — prevents duplicate provider
    // requests if the user double-clicks Run or the parent re-renders while
    // a request is inflight.
    if (loading) return;
    setLoading(true);
    setError(null);
    try {
      const r = await run({ data: { instrument, months, ambiguousPolicy } });
      setResult(r);
      onResult?.(r);
    } catch (e) {
  ...
  return (
    <div data-testid="absolute-validation-panel">
      <div
        style={{
      ...
      <Section title="Historical CSV Import (Stage 5.1)">
        ...
        {comparison && (
          <div style={{ marginTop: 12 }}>
            ...
            <KV k="Overall" v={comparison.overall} />
      ...
      <Section title="Run Historical Validation">
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end" }}>
          ...
          <button
            ...
            disabled={loading}
            aria-busy={loading}
            data-testid="absolute-validation-run"
            style={{
              padding: "8px 16px",
              background: C.gold,
              color: "#000",
...
// Default export enables React.lazy() dynamic import from /backtest so
// none of the CSV parser / validation modules ship in the daily bundle.
export default AbsoluteValidationPanel;