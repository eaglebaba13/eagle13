...
export function computeMarketBias(input: BiasInput, now: number = Date.now()): BiasResult {
  ...
  if (livePrice != null && bundle.freshness !== "UNAVAILABLE") {
    available++;
    if (livePrice > bundle.pivot.r1) {
      score += 2;
      reasons.push("Price above R1");
    } else if (livePrice > bundle.pivot.pp) {
      score += 1;
      reasons.push("Price above PP");
    } else if (livePrice < bundle.pivot.s1) {
      score -= 2;
      reasons.push("Price below S1");
    } else if (livePrice < bundle.pivot.pp) {
      score -= 1;
      reasons.push("Price below PP");
  ...
  checked++;
  if (
    bundle.gann.status !== "UNAVAILABLE" &&
    livePrice != null &&
    bundle.gann.up != null &&
    bundle.gann.down != null
  ) {
    available++;
    const mid = (bundle.gann.up + bundle.gann.down) / 2;
    if (livePrice > mid) {
      score += 1;
      reasons.push("Above Gann midline");
    } else if (livePrice < mid) {
      score -= 1;
      reasons.push("Below Gann midline");
    }
  } else {
  ...
  if (bundle.astro.status !== "UNAVAILABLE" && bundle.astro.levels.length > 0) {
    ...
    if (ups > downs) {
      score += 1;
      reasons.push("Astro bias UP");
    } else if (downs > ups) {
      score -= 1;
      reasons.push("Astro bias DOWN");
    }
  } else {
  ...
  if (input.instrument.id === "XAUUSD" || input.instrument.id === "XAGUSD") {
    ...
    if (macro && macro.macroBias !== "UNAVAILABLE") {
      ...
      if (input.instrument.id === "XAUUSD") {
        if (gold) {
          score += 1;
          reasons.push("Macro ratio favours gold");
        } else if (silver) {
          score -= 1;
          reasons.push("Macro ratio favours silver");
        }
      } else {
        if (silver) {
          score += 1;
  ...
  const confidence = Math.round(
    Math.min(1, Math.max(0, Math.abs(score) / 5) * dataCompleteness) * 100,
  );
