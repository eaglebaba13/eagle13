...
function niftyBundle(): LevelBundle {
  return {
    ...
    gann: {
      ...
      down: 95,
      sourcePrice: 105,
      formulaVersion: "GANN_SQUARE_OF_9_V1",
      status: "FRESH",
    },
    astro: {
      status: "FRESH",
      ...
      generatedAt: candle.candleOpenTime,
      validForDate: "2026-07-21",
    },
    calculatedAt: candle.candleOpenTime,
    freshness: "FRESH",
  };
}
...
describe("computeMarketBias", () => {
  it("returns bullish when price above R1 with Gann/Astro up", () => {
    const r = computeMarketBias({
      instrument: getInstrument("NIFTY"),
      bundle: niftyBundle(),
      livePrice: 118,
    });
