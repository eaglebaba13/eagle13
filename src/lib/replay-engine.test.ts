...
describe("visibleCandles / causality", () => {
  const cs = mkCandles([[1, 2, 0, 1.5, 10], [1.5, 3, 1, 2, 10], [2, 2.5, 1.5, 2.2, 10]]);
...
describe("resolveTrade", () => {
  ...
  it("marks INVALID_SETUP without target/stop", () => {
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "next_open",
      target: null, stop: null, candles: cs, currentIndex: 2, policy: "conservative",
    });
  ...
  it("entry-mode next_open enters at next candle open", () => {
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "next_open",
      target: 104.5, stop: 99, candles: cs, currentIndex: 2, policy: "conservative",
    });
  ...
  it("hits stop when low breaks stop", () => {
    const bear = mkCandles([[100, 101, 99, 100, 10], [100, 100, 95, 96, 10]]);
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "signal_close",
      target: 110, stop: 97, candles: bear, currentIndex: 1, policy: "conservative",
    });
  ...
  it("both-touched → conservative policy maps to STOP", () => {
    const both = mkCandles([[100, 100, 100, 100, 10], [100, 110, 90, 100, 10]]);
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "next_open",
      target: 105, stop: 95, candles: both, currentIndex: 1, policy: "conservative",
    });
  ...
  it("both-touched → optimistic policy maps to TARGET", () => {
    ...
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "next_open",
      target: 105, stop: 95, candles: both, currentIndex: 1, policy: "optimistic",
    });
    expect(r.ambiguous).toBe(true);
    expect(r.status).toBe("TARGET_HIT");
  });
  ...
  it("remains ACTIVE while no target/stop hit yet", () => {
    const flat = mkCandles([[100, 101, 99, 100, 10], [100, 101, 99, 100, 10]]);
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "signal_close",
      target: 200, stop: 50, candles: flat, currentIndex: 1, policy: "conservative",
    });
  ...
  it("tracks MFE and MAE correctly", () => {
    ...
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "signal_close",
      target: 200, stop: 50, candles: path, currentIndex: 2, policy: "conservative",
    });
  ...
  it("does not read future candles beyond currentIndex", () => {
    ...
    const r = resolveTrade({
      signal: "BUY", signalIndex: 0, entryMode: "signal_close",
      target: 150, stop: 50, candles: path, currentIndex: 1, policy: "conservative",
    });
    expect(r.status).toBe("ACTIVE");
    expect(r.exit).toBeNull();
  });
...
describe("computeReplayRunId", () => {
  const cfg: ReplayConfig = {
    symbol: "NIFTY50", date: "2024-01-05", timeframe: "5m",
    provider: "yahoo", entryMode: "next_open", policy: "conservative",
