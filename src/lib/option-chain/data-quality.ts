...
export function assessDataQuality(
  ...
  if (!snapshot.expiry)
    issues.push({ code: "MISSING_EXPIRY", severity: "FAIL", detail: "expiry not present" });
  if (snapshot.spotPrice == null)
    issues.push({ code: "SPOT_MISSING", severity: "FAIL", detail: "spot price null" });
  ...
  for (const s of snapshot.strikes) {
    if (seen.has(s.strike)) {
      issues.push({
        ...
        severity: "FAIL",
        detail: `strike ${s.strike} duplicated`,
      });
    }
    seen.add(s.strike);
    const co = s.call.oi;
  ...
  if (zeroOi > 0)
    issues.push({
      code: "ZERO_OI",
      severity: "WARN",
      detail: `${zeroOi} strikes with zero OI on both legs`,
    });
  if (negativeOi > 0)
    issues.push({
      code: "NEGATIVE_OI",
      severity: "FAIL",
      detail: `${negativeOi} strikes with negative OI`,
    });
  if (partial > 0)
    issues.push({
      code: "PARTIAL_SNAPSHOT",
      severity: "WARN",
      detail: `${partial} strikes missing OI`,
    });

  if (snapshot.strikes.length < minStrikes) {
    issues.push({
      code: "INSUFFICIENT_STRIKES",
      severity: "FAIL",
      detail: `only ${snapshot.strikes.length} strikes`,
    });
  }
  ...
  if (!Number.isFinite(ts)) {
    issues.push({
      ...
      severity: "FAIL",
      detail: "snapshot timestamp not parseable",
    });
  } else {
    if (ts > now + futureToleranceMs) {
      issues.push({
        ...
        severity: "FAIL",
        detail: "snapshot timestamp is in the future",
      });
    }
    if (now - ts > staleMs) {
      issues.push({
        code: "PROVIDER_STALE",
        severity: "WARN",
        detail: `snapshot older than ${Math.round(staleMs / 1000)}s`,
      });
    }
  }
