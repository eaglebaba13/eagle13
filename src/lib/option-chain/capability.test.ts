...
function goodSnap(over: Partial<OptionChainSnapshot> = {}): OptionChainSnapshot {
  const strikes = [24000, 24100, 24200, 24300, 24400, 24500].map((s) =>
    makeStrike(s, { oi: 1000 }, { oi: 1000 }),
  );
  return {
    instrument: "NIFTY",
    spotPrice: 24250,
...
describe("option-chain capability", () => {
  it("valid supported snapshot", () => {
    const snap = goodSnap();
    const c = evaluateOptionChainCapability(
      ev({
        snapshot: snap,
        quality: assessDataQuality(snap, { nowIso: "2026-07-17T04:00:05.000Z" }),
      }),
    );
  ...
  it("insufficient strikes → PARTIAL_CHAIN", () => {
    const snap = goodSnap({
      strikes: [makeStrike(24000, { oi: 1 }, { oi: 1 }), makeStrike(24100, { oi: 1 }, { oi: 1 })],
    });
    const c = evaluateOptionChainCapability(
      ev({
        snapshot: snap,
  ...
  it("duplicate strikes → DATA_QUALITY_FAILURE", () => {
    const snap = goodSnap({
      strikes: [24000, 24000, 24100, 24200, 24300, 24400].map((s) =>
        makeStrike(s, { oi: 1 }, { oi: 1 }),
      ),
    });
    const q = assessDataQuality(snap, { nowIso: "2026-07-17T04:00:05.000Z" });
    const c = evaluateOptionChainCapability(ev({ snapshot: snap, quality: q }));
