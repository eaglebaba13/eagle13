...
describe("load-simulator", () => {
  it("50 users with 70% cache is SAFE", () => {
    const r = simulateLoad({
      ...
      cacheHitRatio: 0.7,
      providerP95LatencyMs: 400,
      providerCapacityRps: 100,
    });
  ...
  it("standard tiers cover 50..1000", () => {
    const reports = simulateStandardTiers({
      requestsPerUserPerMinute: 30,
      cacheHitRatio: 0.8,
      providerP95LatencyMs: 300,
      providerCapacityRps: 200,
    });
  ...
  it("p95 inflates near saturation", () => {
    const light = simulateLoad({
      users: 10,
      requestsPerUserPerMinute: 6,
      cacheHitRatio: 0.5,
      providerP95LatencyMs: 200,
      providerCapacityRps: 100,
    });
