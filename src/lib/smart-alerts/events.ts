...
function directionalTransition(
  ...
  if ((prev === "BULLISH" && cur === "BEARISH") || (prev === "BEARISH" && cur === "BULLISH"))
    return "REVERSAL";
  return "TILT";
}
...
export function generateAlertEvents(
  ...
  if (
    ctx.pcr.available &&
    ctx.pcr.direction &&
    ctx.pcr.direction !== prior.pcrDirection &&
    prior.pcrDirection
  ) {
    out.push(
      buildEvent({
  ...
  if (
    ctx.vix.available &&
    ctx.vix.regime !== "UNKNOWN" &&
    prior.vixRegime &&
    prior.vixRegime !== "UNKNOWN" &&
    prior.vixRegime !== ctx.vix.regime
  ) {
    out.push(
      buildEvent({
        ...base,
    ...
    if (lvl.touched || Math.abs(lvl.distancePoints) <= cfg.gannTouchTolerancePoints) {
      out.push(
        buildEvent({
          ...
          canonicalEntity: `GANN:${lvl.instrument}:${lvl.closestLabel}`,
      ...
      out.push(
        buildEvent({
          ...
          instrument: lvl.instrument,
          canonicalEntity: `GANN:${lvl.instrument}:${lvl.closestLabel}:APPROACH`,
  ...
  if (
    ...
    ctx.ai.bias !== "UNKNOWN" &&
    prior.aiBias &&
    prior.aiBias !== "UNKNOWN" &&
    prior.aiBias !== ctx.ai.bias
  ) {
    out.push(
      buildEvent({
        ...base,
    ...
    if (prev !== "STALE" && cur === "STALE") {
      out.push(
        buildEvent({
          ...
          evidence: [
            {
              module: module as AlertSourceModule,
              previous: prev,
              current: cur,
              freshness: cur,
              available: false,
