...
import type { HistoricalRow, ReadinessState, SignalEvent, SignalFamily } from "./types";
...
export function eventsForRow(row: HistoricalRow): readonly SignalEvent[] {
  ...
  if (row.decision) {
    ...
    const predicted = state === "BULLISH" ? "GAP_UP" : state === "BEARISH" ? "GAP_DOWN" : null;
    events.push({
      family: "DECISION",
      label: state,
      symbol: row.symbol,
  ...
  if (row.gannGap) {
    ...
    const predicted = outlook === "GAP_UP" ? "GAP_UP" : outlook === "GAP_DOWN" ? "GAP_DOWN" : null;
    ...
    events.push({
      ...
      readiness: readiness(
        outlook !== "UNAVAILABLE",
        outlook === "NO_TRADE" || outlook === "CONFLICT",
      ),
      confidence: null,
      blockingWarnings: [],
      eligible,
  ...
  for (const alert of row.smartAlerts) {
    events.push({
      ...
      confidence: null,
      blockingWarnings: alert.readinessBlocked
        ? ["READINESS_BLOCKED"]
        : alert.staleData
          ? ["STALE_DATA"]
          : [],
  ...
  if (row.institutionalFlow) {
    ...
    const predicted =
      summary === "PUT_WRITERS_ACTIVE"
        ? "GAP_UP"
        : summary === "CALL_WRITERS_ACTIVE"
          ? "GAP_DOWN"
          : null;
    events.push({
      ...
      readiness: readiness(
        summary !== "UNAVAILABLE",
        summary === "CONFLICT" || summary === "BALANCED",
      ),
      confidence: null,
      blockingWarnings: [],
