...
import type { ParameterSpec, SensitivityCell } from "./parameter-sensitivity";
import type { OptimizerAggregateInputs, OptimizerStrategyId } from "./explainable-optimizer";
...
export function inspectResearchContext(ctx: Partial<ResolvedResearchContext> | null | undefined): {
  ...
  if (!ctx.formulaVersion)
    gaps.push({ key: "formulaVersion", message: "Formula version is required." });
  ...
  if (!ctx.aggregate) {
    gaps.push({
      key: "aggregate",
      message: "Walk-forward / Monte Carlo / robustness aggregate is required.",
    });
  }
...
export function researchContextSignature(ctx: ResolvedResearchContext): string {
  const spaceKey = ctx.parameterSpace.map((s) => `${s.name}:${s.min}:${s.max}:${s.step}`).join(",");
