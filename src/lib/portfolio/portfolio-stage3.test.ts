...
describe("Phase 22 Stage 3 · auto-registration", () => {
  ...
  it("rejects overfit / unreliable / negative-edge candidates", () => {
    ...
    expect(evaluateRegistrationSafety(r, { overfitStatus: "OVERFIT" }, 1)).toBe(
      "OPTIMIZER_OVERFIT",
    );
    expect(evaluateRegistrationSafety(r, { reliability: "POOR" }, 1)).toBe(
      "UNRELIABLE_RECOMMENDATION",
    );
    expect(evaluateRegistrationSafety(r, { oosExpectancy: -0.1 }, 1)).toBe(
  ...
  it("rejects incomplete metadata", () => {
    ...
    expect(evaluateRegistrationSafety({ ...r, trades: [] }, undefined, 5)).toBe(
      "INSUFFICIENT_TRADES",
    );
  });
...
describe("Phase 22 Stage 3 · efficient frontier", () => {
  ...
  it("target return / vol portfolios respect their constraint", () => {
    ...
    if (f.targetReturnPortfolio)
      expect(f.targetReturnPortfolio.expectedReturn).toBeGreaterThanOrEqual(0);
...
describe("Phase 22 Stage 3 · portfolio recommendation", () => {
  ...
  it("cannot recommend a scenario that breaches hard gates", () => {
    ...
    const r = runPortfolioResearch({
      candidates: [bad, B],
      config: { ...cfg, constraints: { ...cfg.constraints } },
    });
...
describe("Phase 22 Stage 3 · exports carry provenance", () => {
  it("Frontier / Risk Budget / Recommendation / Comparison / Treemap CSVs contain disclaimer", () => {
    ...
    for (const csv of [
      buildFrontierCsv(f),
      buildRiskBudgetCsv(rb),
      buildRecommendationCsv(rec),
      buildScenarioComparisonCsv(cmp),
      tree,
    ]) {
