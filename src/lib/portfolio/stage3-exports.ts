...
export function buildFrontierCsv(front: FrontierResult): string {
  ...
  const emit = (label: string, p: FrontierResult["feasible"][number] | null) => {
    ...
    rows.push(
      [
        label,
        ...p.weights.map((w) => w.toFixed(6)),
        p.expectedReturn.toFixed(6),
        p.volatility.toFixed(6),
        p.sharpe.toFixed(6),
        p.diversificationRatio.toFixed(6),
        p.efficient,
        p.dominated,
      ]
        .map(csvEscape)
  ...
  return (
    header([
      `# method=${front.method}`,
      `# combinationsExplored=${front.combinationsExplored}`,
...
export function buildRiskBudgetCsv(rb: RiskBudgetResult): string {
  ...
  for (const r of rb.rows)
    rows.push(
      [r.key, r.target.toFixed(6), r.actual.toFixed(6), r.gap.toFixed(6), r.breach, r.suggestion]
        .map(csvEscape)
        .join(","),
    );
  return (
    header([
      ...
      `# compliance=${rb.compliance.toFixed(4)}`,
    ]) +
    rows.join("\n") +
    "\n"
  );
...
export function buildScenarioComparisonCsv(cmp: ScenarioComparisonResult): string {
  ...
  for (const r of cmp.rows)
    rows.push(cols.map((c) => csvEscape((r as unknown as Record<string, unknown>)[c])).join(","));
  return header(cmp.warnings.map((w) => `# WARNING ${w}`)) + rows.join("\n") + "\n";
}

export function buildAllocationTreemapCsv(
  result: PortfolioResearchResult,
  assets: { id: string; strategy: string; instrument: string; timeframe: string }[],
): string {
