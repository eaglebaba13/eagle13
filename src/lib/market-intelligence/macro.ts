...
export function classifyMacroRisk(inputs: readonly MacroInput[]): {
  ...
  if (dxy != null) {
    if (dxy > 0.75) {
      score += 2;
      reasons.push("DXY surging");
    } else if (dxy > 0.3) {
      score += 1;
      reasons.push("DXY firm");
    } else if (dxy < -0.5) {
      score -= 1;
      reasons.push("DXY softening");
    }
  }
  ...
  if (us10y != null) {
    if (us10y > 3) {
      score += 2;
      reasons.push("US10Y spike");
    } else if (us10y > 1) {
      score += 1;
      reasons.push("US10Y rising");
    }
  }
  ...
  if (crude != null) {
    if (crude > 3) {
      score += 2;
      reasons.push("Crude spike");
    } else if (crude > 1.5) {
      score += 1;
      reasons.push("Crude firm");
    }
  }
  const usdinr = by.get("USDINR")?.changePct;
  if (usdinr != null && usdinr > 0.3) {
    score += 1;
    reasons.push("INR weakening");
  }
