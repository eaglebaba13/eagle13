...
export function computeInstitutionalProbability(input: ProbabilityInputs): ProbabilityResult {
  ...
  if (input.institutionalBias) {
    ...
    if (v > 0) bull += WEIGHTS.bias * v;
    else if (v < 0) bear += WEIGHTS.bias * -v;
    reasons.push(`Institutional bias ${input.institutionalBias}`);
  } else missing.push("institutionalBias");

  if (input.macroRisk) {
    if (input.macroRisk === "HIGH") {
      bear += WEIGHTS.macro * 0.9;
      reasons.push("Macro risk HIGH");
  ...
  if (input.sectorRotationScore != null) {
    ...
    if (v > 0) bull += WEIGHTS.sector * v;
    else if (v < 0) bear += WEIGHTS.sector * -v;
  ...
  if (input.globalCompositeBiasPct != null) {
    ...
    if (v > 0) bull += WEIGHTS.global * v;
    else if (v < 0) bear += WEIGHTS.global * -v;
  } else missing.push("globalCompositeBiasPct");

  if (input.vix != null) {
    if (input.vix >= 20) {
      bear += WEIGHTS.vix * 0.8;
      reasons.push(`VIX elevated (${input.vix.toFixed(1)})`);
  ...
  if (input.breadthAdvanceDeclinePct != null) {
    ...
    if (v > 0) bull += WEIGHTS.breadth * v;
    else if (v < 0) bear += WEIGHTS.breadth * -v;
  ...
  if (posNews || negNews) {
    ...
    if (net > 0) bull += WEIGHTS.news * net;
    else if (net < 0) bear += WEIGHTS.news * -net;
  }
  ...
  if (input.globalCompositeBiasPct != null && input.institutionalBias) {
    const dirBias = input.institutionalBias.includes("BUY")
      ? 1
      : input.institutionalBias.includes("SELL")
        ? -1
        : 0;
    if (
      dirBias &&
      Math.sign(input.globalCompositeBiasPct) &&
      dirBias !== Math.sign(input.globalCompositeBiasPct)
    ) {
  ...
  const usable =
    ...
    missing.reduce((s, k) => {
      const w: Record<string, number> = {
        institutionalBias: WEIGHTS.bias,
        macroRisk: WEIGHTS.macro,
        sectorRotationScore: WEIGHTS.sector,
        globalCompositeBiasPct: WEIGHTS.global,
        vix: WEIGHTS.vix,
        breadthAdvanceDeclinePct: WEIGHTS.breadth,
        pcr: WEIGHTS.pcr,
      };
      return s + (w[k] ?? 0);
    }, 0);
