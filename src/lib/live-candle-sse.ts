...
// NO polling. NO setInterval. Token never exposed.
 ...
 * Tick-driven: registers as a subscriber to the canonical live market stream.
 * NO polling loop. Updates arrive only when provider ticks modify candles.
 */
export function createLiveCandleSSE(symbol: string, intervalMs: number): Response {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Keepalive comment every30s to prevent proxy timeout
      // This is a comment frame, NOT a data event
      keepaliveId = setInterval(() => {
        if (!closed) {
          try { controller.enqueue(writeSSEComment("keepalive")); } catch { closed = true; }
        }
...
export function createLiveCandleSSE(symbol: string, intervalMs: number): Response {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      }, 30 Exposed to client via SSE — only candle/status data.
import type { QuoteSymbol } from "./provider-foundation/types";
import { INDSTOCKS_SUPPORTED_SYMBOLS } from "./provider-foundation/indstocks/indstocks-instruments.server";
import type { AggregationIntervalMs } from "./provider-foundation/indstocks/candle-aggregator";
import type { LiveCandleEvent } from "./live-market-stream.server";
