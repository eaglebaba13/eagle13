...
function mkReport(mode: AuditReport["mode"], planets: PlanetComparison[]): AuditReport {
  return {
    ...
    mode,
    provisionalDefault: {
      status: "PROVISIONAL METHODOLOGY DEFAULT",
      nodeMode: "mean",
    ...
    summary: {
      ...
      exact: planets.length,
      acceptable: 0,
      warning: 0,
      fail: 0,
      nakshatraMismatches: 0,
      padaMismatches: 0,
      retroMismatches: 0,
      maxLevelDelta: 0,
      levelsChanged: 0,
...
describe("Phase 21.0C · aggregates & coverage", () => {
  it("aggregates per-mode per-planet stats", () => {
    const reports = [
      mkReport("SWISS_LAHIRI_MEAN_GEOCENTRIC", [
        mkComp("Sun"),
        mkComp("Moon", { diffDeg: 0.2, toleranceStatus: "WARNING" }),
      ]),
      mkReport("SWISS_LAHIRI_MEAN_GEOCENTRIC", [
        mkComp("Sun", { diffDeg: 0.1, toleranceStatus: "ACCEPTABLE" }),
      ]),
    ];
