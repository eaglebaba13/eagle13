...
export function composeExplanation(signals: CanonicalSignals, out: StrategyEngineOutput): string {
  const parts: string[] = [];
  ...
  if (top) {
    parts.push(
      `Therefore ${top.profile.label} is preferred (alignment ${top.alignmentPct}%, confidence ${out.direction.confidence}%).`,
    );
  } else {
