...
import type { MarketBreadthSnapshot, PcrConfirmation, VixRegimeReading } from "./types";
...
function pcr(
  available: boolean,
  freshness: "FRESH" | "STALE" | "UNKNOWN" = "FRESH",
  quality: PcrConfirmation["dataQuality"] = "OK",
): PcrConfirmation {
  return {
    available,
...
function snap(dataQuality: MarketBreadthSnapshot["dataQuality"]): MarketBreadthSnapshot {
  return {
    ...
    netBreadth: 10,
    weightedBreadth: 0.2,
    weightedAdvance: null,
    weightedDecline: null,
    weightedUnchanged: null,
    totalWeight: null,
    freshness: "FRESH",
    dataQuality,
...
describe("evaluateMarketBreadthCapability", () => {
  ...
  it("PARTIAL when PCR unavailable", () => {
    const c = evaluateMarketBreadthCapability({
      ...base,
      vix: vix(15),
      pcr: pcr(false, "UNKNOWN", "UNAVAILABLE"),
      breadth: { broad: snap("OK"), nifty50: snap("OK") },
      breadthSource: "LIVE",
    });
    expect(c.status).toBe("PARTIAL");
  ...
  it("DATA_QUALITY_FAILURE when all breadth failed", () => {
    const c = evaluateMarketBreadthCapability({
      ...
      vix: vix(15),
      pcr: pcr(true),
      breadth: { broad: snap("FAILED"), nifty50: snap("FAILED") },
      breadthSource: "LIVE",
    });
