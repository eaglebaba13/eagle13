...
export function detectConflicts(inp: ConflictInputs): readonly ConflictItem[] {
  ...
  const sectorMajorityBearish =
    [inp.banking, inp.it, inp.oilGas, inp.auto].filter((s) => directionOfBreadth(s) === "BEARISH")
      .length >= 3;
  const sectorMajorityBullish =
    [inp.banking, inp.it, inp.oilGas, inp.auto].filter((s) => directionOfBreadth(s) === "BULLISH")
      .length >= 3;

  if (broad === "BULLISH" && top === "BEARISH") {
    conflicts.push({
      code: "BROAD_VS_WEIGHTED",
      message: "Broad breadth bullish but weighted breadth bearish",
    });
  }
  if (broad === "BEARISH" && top === "BULLISH") {
    conflicts.push({
      code: "BROAD_VS_WEIGHTED_REV",
      message: "Broad breadth bearish but weighted breadth bullish",
    });
  }
  if (nifty === "BULLISH" && bank === "BEARISH") {
    conflicts.push({
      code: "NIFTY_VS_BANKING",
      message: "NIFTY breadth bullish but Banking sector bearish",
    });
  }
  if (pcr === "BULLISH" && sectorMajorityBearish) {
    conflicts.push({
      code: "PCR_VS_SECTORS",
      message: "PCR bullish but majority of sectors bearish",
    });
  }
  if (pcr === "BEARISH" && directionOfBreadth(inp.topWeighted) === "BULLISH") {
    conflicts.push({
      code: "PCR_VS_TOP_WEIGHTED",
      message: "PCR bearish but weighted top stocks bullish",
    });
  }
  if (inp.vix.rising && (broad === "BEARISH" || sectorMajorityBearish)) {
    conflicts.push({
      code: "VIX_RISING_BREADTH_WEAK",
      message: "India VIX rising while breadth weakens",
    });
  }
  // Freshness mismatch
  const breadthStale =
    inp.broad?.freshness === "STALE" ||
    inp.nifty50?.freshness === "STALE" ||
    inp.topWeighted?.freshness === "STALE";
  const pcrStale = inp.pcr.freshness === "STALE";
  if (breadthStale && !pcrStale && inp.pcr.available) {
    conflicts.push({
      code: "STALE_BREADTH_FRESH_PCR",
      message: "Breadth is stale but PCR is fresh",
    });
  }
  if (!breadthStale && pcrStale) {
    conflicts.push({
      code: "FRESH_BREADTH_STALE_PCR",
      message: "Breadth is fresh but PCR is stale",
    });
  }
