...
function pcrReading(
  state: CombinedPcrReading["confirmedState"],
  score: number,
): CombinedPcrReading {
  return {
    combinedScore: score,
...
describe("GTI classifier", () => {
  it("bullish agreement → CE-focused research state", () => {
    ...
    expect(["STRONG_CE_RESEARCH_FOCUS", "CE_RESEARCH_FOCUS", "BULLISH_BUT_CONFLICTED"]).toContain(
      r.state,
    );
  });
  ...
  it("bearish agreement → PE-focused research state", () => {
    ...
    expect(["STRONG_PE_RESEARCH_FOCUS", "PE_RESEARCH_FOCUS", "BEARISH_BUT_CONFLICTED"]).toContain(
      r.state,
    );
  });
