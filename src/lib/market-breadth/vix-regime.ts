...
export function classifyVix(
  vix: number | null,
  t: VixRegimeThresholds = DEFAULT_VIX_THRESHOLDS,
): VixRegime {
...
export function evaluateVixRegime(input: VixRegimeInput): VixRegimeReading {
  ...
  const prevRegime =
    input.previousRegime ?? classifyVix(input.previousVix ?? null, input.thresholds);
  const rising =
    input.currentVix != null && input.previousVix != null
