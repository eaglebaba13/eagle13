...
import { evaluateDecision, DECISION_WEIGHTS, type DecisionEvidenceInput } from "./decision-center";
...
function full(overrides: Partial<DecisionEvidenceInput> = {}): DecisionEvidenceInput {
  return {
    ...
    shadow: {
      runId: "SH:1",
      readiness: "READY_FOR_SCHEDULED_SHADOW",
      accuracy: 0.75,
      calibration: 0.8,
      resolvedTrades: 60,
    },
...
describe("Decision Center — hard gates", () => {
  it("robustness OVERFIT forces NO_GO", () => {
    const r = evaluateDecision(
      full({ robustness: { runId: "RB", score: 0.9, verdict: "OVERFIT" } }),
    );
  ...
  it("data quality failure forces NO_GO", () => {
    const r = evaluateDecision(
      full({ dataQuality: { ok: false, causalityOk: true, dataHash: "x" } }),
    );
    expect(r.state).toBe("NO_GO");
  ...
  it("causality failure forces NO_GO", () => {
    const r = evaluateDecision(
      full({ dataQuality: { ok: true, causalityOk: false, dataHash: "x" } }),
    );
  ...
  it("recommendation UNRELIABLE forces NO_GO", () => {
    const r = evaluateDecision(
      full({ recommendationValidator: { runId: "RV", reliability: 0.2, verdict: "UNRELIABLE" } }),
    );
...
describe("Decision Center — deployment states", () => {
  function scoreAt(target: number): DecisionEvidenceInput {
    ...
    return {
      walkForward: {
        runId: "WF",
        oosExpectancy: (target - 0.5) * 2,
        stabilityScore: target,
        overfitFlag: false,
        totalTrades: 200,
      },
      monteCarlo: {
        runId: "MC",
        worstDrawdownPct: (1 - target) * 0.5,
        medianCagr: 0.1,
        ruinProbability: 0,
      },
      ...
      recommendation: {
        runId: "RC",
        expectedWinRate: target,
        expectedProfitFactor: 1.5,
        confidence: Math.max(target, 0.6),
      },
...
describe("Decision Center — run id determinism & exports", () => {
  it("run id is deterministic for identical evidence", () => {
    const a = full();
    const b = full();
    const ra = evaluateDecision(a);
    const rb = evaluateDecision(b);
  ...
  it("run id changes when a supporting run id changes", () => {
    ...
    expect(computeDecisionRunId(a, evaluateDecision(a))).not.toBe(
      computeDecisionRunId(b, evaluateDecision(b)),
    );
  });
