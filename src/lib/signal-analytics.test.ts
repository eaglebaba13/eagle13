...
function fakeResult(trades: BacktestTrade[]): BacktestResult {
  return {
    ...
    summary: {
      ...
      taken: trades.filter((t) => t.result === "WIN" || t.result === "LOSS" || t.result === "FLAT")
        .length,
    ...
    dataQuality: {
      expectedSessions: trades.length,
      loadedSessions: trades.length,
      missingSessions: 0,
      invalidSessions: 0,
      coveragePct: 100,
      dataSource: "Test",
...
describe("computeAnalytics — drawdown with a peak/trough", () => {
  ...
  const r = {
    ...
    stats: {
      sampleSize: 4,
      sampleWarning: "INSUFFICIENT" as const,
      expectancy: 2.75,
      median: 0,
      stddev: 0,
      sharpeLike: 0,
