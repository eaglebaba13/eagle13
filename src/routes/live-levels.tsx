  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Live Astro Level Terminal | EagleBABA" },
      {
        ...
        content:
          "Real-time Astro support/resistance levels for NIFTY, BANK NIFTY, GOLD, SILVER and BTC.",
      },
...
function exportCsv(m: MarketBlock, version: AstroFormulaVersion = DEFAULT_ASTRO_FORMULA_VERSION) {
  ...
  downloadBlob(
    [
      `# EagleBABA Live Levels · ${version} · R3/S3 = EagleBaba Extended`,
      EXPORT_COLS.join(","),
      ...rows,
    ].join("\n"),
...
function LiveLevelsTerminal() {
  ...
  const [alerts, setAlerts] = useState<
    { id: string; text: string; tone: "up" | "down" | "info"; at: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const savedSound =
      typeof localStorage !== "undefined" ? localStorage.getItem("eb-ll-sound") : null;
  ...
  const lvls = useMemo(
    () => buildLvls(market.planets, market.livePrice, tol),
    [market.planets, market.livePrice, tol],
  );
  ...
  const bias =
    signal.confidence >= 60 ? "Bullish" : signal.confidence <= 40 ? "Bearish" : "Neutral";
  ...
  const pricePosition =
    nearestResistance && market.livePrice > nearestResistance.value
      ? "Above Resistance"
  ...
  useEffect(() => {
    ...
    if (prev) {
      ...
      const at = new Date(data.asOf).toLocaleTimeString("en-GB", {
        hour12: false,
        timeZone: "Asia/Kolkata",
      });
      ...
      if (oldMarket) {
        const prevLvls = buildLvls(
          oldMarket.planets,
          oldMarket.livePrice,
          tolFor(oldMarket.livePrice),
        );
        for (const l of lvls) {
          ...
          if (old.status !== "BROKEN" && l.status === "BROKEN")
            push(
              `${market.name}: price broke ${l.planet} ${l.kind} (${fmt(l.value)})`,
              l.isResistance ? "up" : "down",
            );
        }
      }
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ marginTop: 8, fontSize: 13 }}>
            Connecting to live planetary & multi-market feed…
          </div>
        </div>
      </div>
    );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <span className="ll-mono" style={{ fontSize: 13, color: C.muted }}>
              IST {clock}
            </span>
            <button
              onClick={() => setSound((s) => !s)}
              title={sound ? "Sound on" : "Sound off"}
              style={{ ...ghost(sound ? C.gold : C.muted), padding: "6px 9px" }}
            >
            ...
            <button
              ...
              title="Alerts"
              style={{ ...ghost(C.gold), position: "relative", padding: "6px 9px" }}
            >
              <Bell size={16} />
              {unread > 0 ? (
                <span
            ...
            <Link
              ...
              title="Preview: Absolute-Degree Intraday v1 (paid-course methodology)"
            >
              Absolute · Preview
            </Link>
            <NewsCenter />
        ...
        <div className="ll-tabs no-print" style={{ marginBottom: 16 }}>
          {data.markets.map((m) => {
            ...
            return (
              <button
                key={m.key}
                className={`ll-tab${m.key === active ? " is-active" : ""}`}
              ...
              >
                ...
                <span
                  ...
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: m.key === active ? "#000" : up ? C.green : C.red,
                  }}
                >
                  {up ? "▲" : "▼"}
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <main style={{ flex: 1, minWidth: 0 }}>
            {isFetching ? (
              <div
                className="no-print ll-mono"
                style={{ fontSize: 11, color: C.gold, marginBottom: 8 }}
              >
                ⟳ Refreshing live planetary & price data…
              </div>
            ) : null}
            ...
            >
              ...
              <Stat
                label="Live Price"
                value={<span className="ll-mono">{fmt(market.livePrice)}</span>}
                color={C.blue}
                sub={`Prev ${fmt(market.prevClose)}`}
              />
              <Stat
                label="Today's Change"
                value={
                  <span className="ll-mono" style={{ color: market.change >= 0 ? C.green : C.red }}>
                    {market.change >= 0 ? "▲" : "▼"} {Math.abs(market.change).toFixed(2)}
                  </span>
                }
                sub={`${market.changePct.toFixed(2)}%`}
              />
              <Stat
                label="Bias"
                value={bias}
                color={biasColor}
                sub={`${data.bullCount} bull · ${data.bearCount} bear nak`}
              />
              <Stat
                label="Moon Nakshatra"
                value={data.moonNakshatra}
                sub={`${data.moonSign} ${data.moonDegree.toFixed(1)}°`}
              />
              <Stat
                label="Retrograde"
                value={data.retroCount}
                color={data.retroCount >= 3 ? C.red : C.text}
                sub={`${data.bullRetroCount}▲ / ${data.bearRetroCount}▼`}
              />
              <Stat
                label="Nearest Planet"
                value={nearest?.planet ?? "—"}
                sub={nearest ? `${nearest.kind} · ${Math.round(nearest.distance)} pts` : undefined}
              />
              <Stat
                label="Nearest Support"
                value={
                  nearestSupport ? (
                    <span className="ll-mono">{fmt(nearestSupport.value)}</span>
                  ) : (
                    "—"
                  )
                }
                color={C.green}
                sub={nearestSupport ? `${nearestSupport.planet} ${nearestSupport.kind}` : undefined}
              />
              <Stat
                label="Nearest Resistance"
                value={
                  nearestResistance ? (
                    <span className="ll-mono">{fmt(nearestResistance.value)}</span>
                  ) : (
                    "—"
                  )
                }
                ...
                sub={
                  nearestResistance
                    ? `${nearestResistance.planet} ${nearestResistance.kind}`
                    : undefined
                }
              />
              <Stat
                ...
                sub={`${signal.strength} · ${signal.confidence}%`}
              />
              <Stat
                label="Price Position"
                value={pricePosition}
                color={
                  pricePosition === "Above Resistance"
                    ? C.green
                    : pricePosition === "Below Support"
                      ? C.red
                      : C.gold
                }
              />
              <Stat
                label="Last Updated"
                value={<span className="ll-mono">{lastUpdated}</span>}
                sub="IST · auto 60s"
              />
            </div>
            ...
            <div className="ll-grid" style={{ gridTemplateColumns: "1.2fr 1fr", marginBottom: 14 }}>
              <Card
                style={{
                  background: `linear-gradient(135deg, color-mix(in oklab, ${signalColor} 16%, transparent), ${C.card})`,
              ...
              >
                ...
                >
                  ...
                  >
                    Live Signal Engine · {market.name}
                  </div>
                  <span className="ll-mono" style={{ fontSize: 10, color: C.muted }}>
                    <Radio size={11} style={{ verticalAlign: -1 }} /> every minute
                  </span>
                </div>
                <div
                  style={{
                ...
                <div style={{ fontSize: 13, marginTop: 2 }}>
                  {signal.strength} · Confidence <b>{signal.confidence}%</b>
                </div>
                <div
                  style={{
                    height: 7,
              ...
              <Card
                style={{
                  ...
                  background: `linear-gradient(135deg, color-mix(in oklab, ${biasColor} 14%, transparent), ${C.card})`,
                }}
              >
                <div
                  style={{
                ...
                <div style={{ fontSize: 34, fontWeight: 900, color: biasColor, marginTop: 4 }}>
                  {bias}
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <MiniField
                    label="Base Cycle"
                    value={<span className="ll-mono">{market.cycles.base}</span>}
                  />
                  <MiniField label="Price Position" value={pricePosition} />
                  <MiniField
                    label="Nearest Support"
                    value={nearestSupport ? fmt(nearestSupport.value) : "—"}
                  />
                  <MiniField
                    label="Nearest Resistance"
                    value={nearestResistance ? fmt(nearestResistance.value) : "—"}
                  />
                </div>
              </Card>
            </div>
            ...
            >
              <NearestCard
                title="Nearest Planet"
                planet={nearest?.planet}
                main={nearest ? `${nearest.kind}` : "—"}
                sub={nearest ? `${fmt(nearest.value)} · ${Math.round(nearest.distance)} pts` : ""}
                color={C.blue}
              />
              <NearestCard
                title="Nearest Support"
                planet={nearestSupport?.planet}
                main={nearestSupport ? fmt(nearestSupport.value) : "—"}
                sub={
                  nearestSupport
                    ? `${nearestSupport.planet} ${nearestSupport.kind} · ${Math.round(nearestSupport.distance)} pts`
                    : ""
                }
                color={C.green}
              />
              <NearestCard
                ...
                planet={nearestResistance?.planet}
                main={nearestResistance ? fmt(nearestResistance.value) : "—"}
                sub={
                  nearestResistance
                    ? `${nearestResistance.planet} ${nearestResistance.kind} · ${Math.round(nearestResistance.distance)} pts`
                    : ""
                }
                color={C.red}
              />
              <NearestCard
                ...
                planet={undefined}
                main={pricePosition}
                sub={nearest ? `${nearest.planet} ${nearest.kind}` : ""}
                color={C.gold}
              />
            </div>
                ...
                <table className="ll-table">
                  <thead>
                    <tr>
                      <th>Planet</th>
                      <th>Degree</th>
                      <th style={{ color: C.red }}>R1</th>
                      <th style={{ color: C.red }}>R2</th>
                      <th style={{ color: C.red }}>R3</th>
                      <th style={{ color: C.blue }}>Price</th>
                      <th style={{ color: C.green }}>S1</th>
                      <th style={{ color: C.green }}>S2</th>
                      <th style={{ color: C.green }}>S3</th>
                      <th>Nearest</th>
                      <th>Dist</th>
                      <th>Status</th>
                      <th>Signal</th>
                  ...
                  <tbody>
                    {market.planets.map((p) => {
                      ...
                      return (
                        <tr
                          key={p.planet}
                          className={isNearestRow ? "ll-row-near" : undefined}
                        ...
                        >
                          <td>
                            ...
                            >
                              <span
                                className="eb-planet-orb"
                                style={{ width: 20, height: 20, ...orbStyle(p.planet) }}
                              />
                              {p.planet}
                            </span>
                          </td>
                          <td className="ll-mono">{p.degree.toFixed(2)}°</td>
                          <td className="ll-mono" style={{ color: C.red }}>
                            {fmt(p.r1)}
                          </td>
                          <td className="ll-mono" style={{ color: C.red }}>
                            {fmt(p.r2)}
                          ...
                          <td className="ll-mono" style={{ color: C.blue, fontWeight: 700 }}>
                            {fmt(market.livePrice)}
                          </td>
                          <td className="ll-mono" style={{ color: C.green }}>
                            {fmt(p.s1)}
                          </td>
                          <td className="ll-mono" style={{ color: C.green }}>
                            {fmt(p.s2)}
                          ...
                          <td className="ll-mono" style={{ fontWeight: 700 }}>
                            {near ? `${near.kind} ${fmt(near.value)}` : "—"}
                          </td>
                          <td className="ll-mono">{near ? Math.round(near.distance) : "—"}</td>
                          <td>
                            <span
                              style={{
                                color: near ? STATUS_COLOR[near.status] : C.muted,
                                fontWeight: 700,
                                fontSize: 11,
                              }}
                            >
                          ...
                          <td className="ll-mono" style={{ color: C.blue }}>
                            {near ? `${near.confidence}%` : "—"}
                          </td>
                          <td style={{ color: p.retro ? C.red : C.green, fontWeight: 700 }}>
                            {p.motion}
                          </td>
                          <td>
                            {p.retro ? <RetroBadge /> : <span style={{ color: C.muted }}>—</span>}
                          </td>
                        </tr>
                      );
                    })}
            ...
            <Card style={{ marginBottom: 16 }}>
              <SectionHead
                title={`ASTRO LEVEL MAP · ${market.name}`}
                note={`Live price vs planetary R1/S1 levels`}
                bare
              />
            ...
            >
              <button onClick={() => exportCsv(market, data.formulaVersion)} style={ghost(C.green)}>
                <Download size={14} style={{ verticalAlign: -2 }} /> CSV
              </button>
              <button
                onClick={() => exportExcel(market, data.formulaVersion)}
                style={ghost(C.blue)}
              >
                <FileSpreadsheet size={14} style={{ verticalAlign: -2 }} /> Excel
              </button>
              <button onClick={() => window.print()} style={ghost(C.gold)}>
                <FileText size={14} style={{ verticalAlign: -2 }} /> PDF
              </button>
              <button onClick={() => window.print()} style={ghost(C.muted)}>
                <Printer size={14} style={{ verticalAlign: -2 }} /> Print
              </button>
            </div>
            ...
            <Card style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 8 }}>
                Open the live news center for full breaking-news coverage, filters and AI market
                view.
              </div>
              <NewsCenter />
            </Card>
            ...
            <div
              className="ll-mono"
              style={{ fontSize: 11, color: C.muted, textAlign: "center", marginBottom: 16 }}
            >
              Last updated {lastUpdated} IST · Auto-refresh every 60 seconds · No manual refresh
              required
            </div>
          ...
          <div style={{ padding: 14, overflowY: "auto" }}>
            {alerts.length === 0 ? (
              <div style={{ color: C.muted, fontSize: 13, textAlign: "center", marginTop: 30 }}>
                No alerts yet. You'll be notified when price touches/breaks a level, the Moon
                changes nakshatra, a planet changes sign, or a retrograde starts/ends.
              </div>
            ) : (
              alerts.map((a) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="ll-mono" style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                      {a.at} IST
                    </div>
                  </div>
                );
      ...
      {selected ? (
        <PlanetDrawer
          ...
          price={market.livePrice}
          fmt={fmt}
          lvls={sorted.filter((l) => l.planet === selected.planet)}
...
function SectionHead({ title, note, bare }: { title: string; note?: string; bare?: boolean }) {
  return (
    <div
      style={{
        padding: bare ? "0 0 10px" : "12px 16px",
        borderBottom: bare ? "none" : `1px solid ${C.border}`,
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
        gap: 10,
    ...
    >
      ...
      {note ? (
        <span className="ll-mono" style={{ fontSize: 11, color: C.muted }}>
          {note}
        </span>
      ) : null}
    </div>
...
function MiniField({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div>
      <div
        style={{ fontSize: 9.5, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5 }}
      >
        {label}
      </div>
  ...
  return (
    <Card style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
        {planet ? (
          <span className="eb-planet-orb" style={{ width: 30, height: 30, ...orbStyle(planet) }} />
        ) : null}
        <div>
          <div className="ll-mono" style={{ fontSize: 20, fontWeight: 800, color }}>
            {main}
          </div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          <div
            style={{ fontWeight: 800, fontSize: 15, display: "flex", alignItems: "center", gap: 6 }}
          >
            {p.planet}
            {p.retro ? <RetroBadge /> : null}
          </div>
          <div className="ll-mono" style={{ fontSize: 11, color: C.muted }}>
      ...
      >
        ...
        <MiniField
          label="Motion"
          value={<span style={{ color: p.retro ? C.red : C.green }}>{p.motion}</span>}
        />
      </div>
      <div style={{ marginTop: 8 }}>
        <BullBearBadge bull={p.bull} bear={p.bear} />
      </div>
    </button>
  );
}
...
function BullBearBadge({ bull, bear }: { bull: boolean; bear: boolean }) {
  ...
  return (
    <span
      style={{
        background: bull ? "rgba(16,185,129,0.16)" : "rgba(225,29,72,0.16)",
      ...
      <div className="ll-drawer no-print" style={drawerStyle(400)}>
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
            <span
              className="eb-planet-orb"
              style={{ width: 72, height: 72, ...orbStyle(p.planet) }}
            />
            <div>
              <div
                style={{
          ...
          >
            <MiniField
              ...
              value={<span className="ll-mono">{p.absDegree.toFixed(2)}°</span>}
            />
            ...
            <MiniField
              label="Speed"
              value={<span className="ll-mono">{p.speed.toFixed(4)}</span>}
            />
            <MiniField
              label="Motion"
              value={<span style={{ color: p.retro ? C.red : C.green }}>{p.motion}</span>}
            ...
            {(["r1", "r2", "r3", "s1", "s2", "s3"] as const).map((k) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 8,
                  padding: "8px 10px",
                  borderLeft: `3px solid ${k.startsWith("r") ? C.red : C.green}`,
                }}
              >
                <div style={{ fontSize: 10, color: C.muted, textTransform: "uppercase" }}>{k}</div>
                <div className="ll-mono" style={{ fontWeight: 800 }}>
                  {fmt(p[k])}
                </div>
              </div>
            ))}
          ...
          >
            <div
              style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 1 }}
            >
              Nearest Price Level
            </div>
            {near ? (
              <>
                <div className="ll-mono" style={{ fontSize: 20, fontWeight: 800, marginTop: 4 }}>
                  {near.kind} · {fmt(near.value)}
                </div>
                <div style={{ fontSize: 12, color: C.muted }}>
                  Distance from live price ({fmt(price)}):{" "}
                  <b className="ll-mono" style={{ color: C.text }}>
                    {Math.round(near.distance)} pts
                  </b>
                </div>
                <div style={{ fontSize: 12, marginTop: 4 }}>
                  Status <b style={{ color: STATUS_COLOR[near.status] }}>{near.status}</b> · Signal{" "}
                  <b
                    style={{
                      color:
                        near.signal === "BUY" ? C.green : near.signal === "SELL" ? C.red : C.gold,
                    }}
                  >
                    {near.signal}
                  </b>
        ...
        yaxis: [
          {
            ...
            strokeDashArray: 4,
            label: {
              text: `${market.name} ${fmt(market.livePrice)}`,
              style: { background: "var(--eb-accent)", color: "#000" },
            },
          },
        ],
      },
