  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Live Astro Planet Position Terminal | EagleBABA" },
      {
        ...
        content:
          "Real-time planetary positions and auto-updated astro support/resistance levels for NIFTY.",
      },
...
function exportCsv(
  ...
  const header = [`# EagleBABA Live Astro Levels · ${version} · R3/S3 = EagleBaba Extended`];
...
function LiveTerminal() {
  ...
  useEffect(() => {
    setMounted(true);
    const saved =
      typeof localStorage !== "undefined" ? localStorage.getItem("eb-terminal-location") : null;
  ...
  const lvls = useMemo(
    () => buildLvls(data.planets, data.livePrice),
    ...
    if (prev) {
      ...
      for (const l of lvls) {
        ...
        if (old.status !== "BROKEN" && l.status === "BROKEN")
          push(
            `Price broke ${l.planet} ${l.kind} (${num(l.value)})`,
      ...
      >
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: C.text }}>
            🛰️ Live Astro Planet Position Terminal
          </div>
          <div style={{ marginTop: 8, fontSize: 13 }}>
            Connecting to live planetary & market feed…
          </div>
        </div>
      </div>
    );
    ...
    >
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: C.muted }}>
                <MapPin size={12} style={{ verticalAlign: -2 }} /> {loc.label} · Asia/Kolkata ·
                auto-refresh 60s
              </div>
          ...
          >
            <span className="lt-mono" style={{ fontSize: 13, color: C.muted }}>
              IST {clock}
            </span>
            <span
              style={{
            ...
            <button
              ...
              title="Settings"
              style={{ ...ghost(C.muted), padding: "6px 9px" }}
            >
              <MapPin size={16} />
            </button>
            <Link
        ...
        {showSettings ? (
          ...
          >
            <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1, color: C.muted }}>
              LOCATION
            </span>
            <select
              value={location}
            ...
            <span style={{ fontSize: 11, color: C.muted }}>
              Timezone Asia/Kolkata · Sunrise {sunrise} · Sunset {sunset}
            </span>
          </Card>
        ) : null}
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <main style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              <Stat
                label="Live NIFTY"
                value={<span className="lt-mono">{num(data.livePrice)}</span>}
                color={C.blue}
                sub={`Prev ${num(data.prevClose)}`}
              />
              <Stat
                label="Signal"
                value={
              ...
              <Stat
                ...
                value={
                  nearestSupport ? (
                    <span className="lt-mono">{num(nearestSupport.value)}</span>
                  ) : (
                    "—"
                  )
                }
              ...
              <Stat
                ...
                value={
                  nearestResistance ? (
                    <span className="lt-mono">{num(nearestResistance.value)}</span>
                  ) : (
                    "—"
                  )
                }
            ...
            <div className="lt-grid" style={{ gridTemplateColumns: "1.2fr 1fr", marginBottom: 14 }}>
              ...
              >
                ...
                >
                  ...
                  <span className="lt-mono" style={{ fontSize: 10, color: C.muted }}>
                    <Radio size={11} style={{ verticalAlign: -1 }} /> updates every minute
                  </span>
                </div>
                <div
                  style={{
              ...
              >
                ...
                >
                  ...
                  <MiniField
                    label="Price vs Support"
                    value={
                      nearestSupport
                        ? `${data.livePrice > nearestSupport.value ? "Above" : "Below"}`
                        : "—"
                    }
                  />
                  <MiniField
                    label="Price vs Resistance"
                    value={
                      nearestResistance
                        ? `${data.livePrice > nearestResistance.value ? "Above" : "Below"}`
                        : "—"
                    }
                  />
                </div>
            ...
            >
              <NearestCard
                ...
                sub={nearest ? `${num(nearest.value)} · ${Math.round(nearest.distance)} pts` : ""}
                color={C.blue}
              />
              <NearestCard
                title="Nearest Support"
                planet={nearestSupport?.planet}
                main={nearestSupport ? num(nearestSupport.value) : "—"}
                sub={
                  nearestSupport
              ...
              <NearestCard
                ...
                main={nearestResistance ? num(nearestResistance.value) : "—"}
                sub={
                  nearestResistance
              ...
              <NearestCard
                title="Direction"
                planet={undefined}
                main={nearest ? (nearest.isResistance ? "▲ Toward R" : "▼ Toward S") : "—"}
                  ...
                  <tbody>
                    {data.planets.map((p) => {
                      const strength = Math.max(
                        8,
                        Math.min(100, Math.round((Math.abs(p.speed) / 13) * 100)),
                      );
                      return (
                        <tr key={p.planet} onClick={() => setSelected(p)}>
                          <td>
                            ...
                            >
                              <span
                                ...
                                style={{ width: 22, height: 22, ...orbStyle(p.planet) }}
                              />
                              {p.planet}
                            </span>
                          </td>
                          ...
                          <td className="lt-mono" style={{ color: p.retro ? C.red : C.green }}>
                            {p.speed.toFixed(3)}
                          </td>
                    ...
                    {data.planets.map((p) => {
                      ...
                      return (
                        <tr key={p.planet} onClick={() => setSelected(p)}>
                          ...
                          <td className="lt-mono">{p.degree.toFixed(2)}°</td>
                          <td className="lt-mono" style={{ color: C.red }}>
                            {num(p.r1)}
                          </td>
                          <td className="lt-mono" style={{ color: C.red }}>
                            {num(p.r2)}
                          ...
                          <td className="lt-mono" style={{ color: C.green }}>
                            {num(p.s1)}
                          </td>
                          <td className="lt-mono" style={{ color: C.green }}>
                            {num(p.s2)}
                          ...
                          <td className="lt-mono" style={{ fontWeight: 700 }}>
                            {near ? `${near.kind} ${num(near.value)}` : "—"}
                          </td>
                          ...
                          <td className="lt-mono" style={{ color: C.blue }}>
            ...
            <Card style={{ marginBottom: 16 }}>
              <SectionHead
                title="ASTRO LEVEL MAP"
                note="Live NIFTY vs planetary R1/S1 levels"
                bare
              />
            ...
            >
              <button
                onClick={() => exportCsv(data.planets, data.formulaVersion)}
                style={ghost(C.green)}
              >
                <Download size={14} style={{ verticalAlign: -2 }} /> CSV
              </button>
              <button
                onClick={() => exportExcel(data.planets, data.formulaVersion)}
                style={ghost(C.blue)}
              >
              ...
              alerts.map((a) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="lt-mono" style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                      {a.at} IST
                    </div>
                  </div>
                );
...
function SectionHead({ title, note, bare }: { title: string; note?: string; bare?: boolean }) {
  return (
    ...
    >
      ...
      {note ? (
        <span className="lt-mono" style={{ fontSize: 11, color: C.muted }}>
          {note}
        </span>
      ) : null}
    </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span
          style={{
            fontSize: 9,
            fontWeight: 700,
            color: ix.marketState === "OPEN" ? C.green : C.muted,
          }}
        >
          {ix.marketState === "OPEN" ? "● LIVE" : "○ CLOSED"}
        </span>
      </div>
      <div className="lt-mono" style={{ fontSize: 22, fontWeight: 800, marginTop: 4 }}>
        {num(ix.livePrice)}
      </div>
  ...
  return (
    <Card style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
        ...
        <div>
          <div className="lt-mono" style={{ fontSize: 22, fontWeight: 800, color }}>
            {main}
          </div>
        ...
        <div style={{ padding: 18, overflowY: "auto" }}>
          ...
          >
            <MiniField
              ...
              value={<span className="lt-mono">{p.absDegree.toFixed(2)}°</span>}
            />
            ...
            <MiniField
              label="Speed"
              value={<span className="lt-mono">{p.speed.toFixed(4)}</span>}
            />
            <MiniField
              label="Motion"
          ...
          >
            {(["r1", "r2", "r3", "s1", "s2", "s3"] as const).map((k) => (
              ...
              >
                ...
                <div className="lt-mono" style={{ fontWeight: 800 }}>
                  {num(p[k])}
                </div>
              </div>
            ))}
          ...
          >
            ...
            {near ? (
              <>
                <div className="lt-mono" style={{ fontSize: 20, fontWeight: 800, marginTop: 4 }}>
                  {near.kind} · {num(near.value)}
                </div>
                <div style={{ fontSize: 12, color: C.muted }}>
                  Distance from live price ({num(price)}):{" "}
                  <b className="lt-mono" style={{ color: C.text }}>
