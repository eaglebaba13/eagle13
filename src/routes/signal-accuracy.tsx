  ...
  head: () => ({
    meta: [
      { title: "Signal Accuracy Analytics | EagleBABA Astro Levels" },
      {
        name: "description",
        content:
          "Institutional-grade analytics on the accuracy of EagleBABA astro BUY / SELL / WAIT signals across Nakshatras, Moon Signs, retrograde combinations, weekdays and months.",
      },
      { property: "og:title", content: "Signal Accuracy Analytics | EagleBABA" },
      {
        property: "og:description",
        content: "Which astro signals historically produce the highest-probability trades.",
      },
...
function SignalAccuracyPage() {
  ...
  const filteredResult = useMemo<BacktestResult | null>(() => {
    ...
    const rk = (n: number) =>
      n <= 0 ? "0 Retro" : n === 1 ? "1 Retro" : n === 2 ? "2 Retro" : "3+ Retro";
    const mNames = [
      "Jan",
      "Feb",
      "Mar",
  ...
  const yearOptions = useMemo(
    () => uniq(result?.trades.map((t) => t.date.slice(0, 4)) ?? []),
    [result],
  );
  ...
  const exportJson = () => {
    ...
    downloadBlob(
      ...
      `eaglebaba-signal-accuracy-${symbol}-${slug}-${from}-${to}.json`,
      "application/json",
    );
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, padding: "18px 16px 96px" }}>
      ...
      >
        <div>
          ...
          <div style={{ fontFamily: "var(--eb-mono)", fontSize: 11, color: C.muted, marginTop: 4 }}>
            Institutional analytics on top of the validated Historical Backtest Engine · no
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Link
            to="/backtest"
            style={{ color: C.blue, fontFamily: "var(--eb-mono)", fontSize: 12 }}
          >
            ← Backtest
          </Link>
      ...
      {!result && !loading ? (
        ...
        >
          Pick an instrument and period, then run the analytics engine to explore historical signal
          accuracy.
        </section>
      ) : null}
    ...
    <section style={{ ...panel, marginTop: 14 }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={tableStyle}>
          <thead>
            <tr>
              {["Signal", "Correct", "Failed", "Flat", "Total", "Accuracy"].map((h) => (
                <th key={h} style={th}>
                  {h}
                </th>
              ))}
            ...
            {a.confusion.map((c) => (
              <tr key={c.signal} style={rowStyle}>
                <td
                  style={{
                    ...td,
                    color: c.signal === "BUY" ? C.green : c.signal === "SELL" ? C.red : C.orange,
                    fontWeight: 700,
                  }}
                >
                  {c.signal}
                ...
                <td
                  style={{
                    ...td,
                    color: c.accuracy >= 55 ? C.green : c.accuracy <= 45 ? C.red : C.text,
                    fontWeight: 700,
                  }}
                >
                  {pct(c.accuracy)}
...
function EquityDrawdownSection({ r, a }: { r: BacktestResult; a: Analytics }) {
  ...
  let peak = equity[0];
  const ddSeries = equity.map((v) => {
    if (v > peak) peak = v;
    return Math.round((v - peak) * 100) / 100;
  });
  return (
    <section style={{ ...panel, marginTop: 14 }}>
      ...
      >
        <div style={miniCard}>
          <div style={miniLbl}>Max Drawdown</div>
          <div style={{ ...miniVal, color: C.red }}>{num(-a.drawdown.maxDrawdown)}</div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Avg Drawdown</div>
          <div style={miniVal}>{num(-a.drawdown.avgDrawdown)}</div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Recovery Days</div>
          <div style={miniVal}>
            {a.drawdown.recoveryDays == null ? "—" : String(a.drawdown.recoveryDays)}
          </div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>New Peaks</div>
          <div style={miniVal}>{a.drawdown.peaks}</div>
        </div>
      </div>
    </section>
  );
...
function BenchmarkSection({ r }: { r: BacktestResult }) {
  ...
  return (
    <section style={{ ...panel, marginTop: 14 }}>
      ...
      >
        <div style={miniCard}>
          <div style={miniLbl}>Buy &amp; Hold</div>
          <div style={{ ...miniVal, color: b.buyAndHoldPct >= 0 ? C.green : C.red }}>
            {pct(b.buyAndHoldPct)}
          </div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Strategy</div>
          <div style={{ ...miniVal, color: b.strategyPct >= 0 ? C.green : C.red }}>
            {pct(b.strategyPct)}
          </div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Excess</div>
          <div style={{ ...miniVal, color: b.excessPct >= 0 ? C.green : C.red }}>
            {pct(b.excessPct)}
          </div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Active Days</div>
          <div style={miniVal}>{b.activeDays}</div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Sharpe</div>
          <div style={miniVal}>{r.stats.sharpeLike}</div>
        </div>
        <div style={miniCard}>
          <div style={miniLbl}>Std Dev</div>
          <div style={miniVal}>{r.stats.stddev}</div>
        </div>
      </div>
    </section>
  );
        ...
        {insights.map((i) => (
          ...
          >
            ...
            <div
              style={{
                ...miniVal,
                color: i.tone === "bull" ? C.green : i.tone === "bear" ? C.red : C.text,
                fontSize: 13,
              }}
            >
              {i.detail}
...
function Disclaimer() {
  return (
    ...
    >
      Analytics are computed from historical backtest results generated by the validated Historical
      Backtest Engine. Past performance does not guarantee future returns. Signal accuracy depends
      on candle resolution, execution assumptions, data quality, slippage and costs.
    </section>
  );
}
...
function BucketTable({ rows, keyLabel }: { rows: Bucket[]; keyLabel: string }) {
  if (rows.length === 0) {
    return (
      <div style={{ color: C.muted, fontFamily: "var(--eb-mono)", fontSize: 12 }}>No data.</div>
    );
  }
  return (
    <div style={{ overflowX: "auto" }}>
          ...
          {rows.map((b) => (
            <tr key={b.key} style={rowStyle}>
              ...
              <td
                style={{
                  ...td,
                  color: b.accuracy >= 55 ? C.green : b.accuracy <= 45 ? C.red : C.text,
                  fontWeight: 700,
                }}
              >
                {pct(b.accuracy)}
              ...
              <td style={{ ...td, color: b.avgReturn >= 0 ? C.green : C.red }}>
                {num(b.avgReturn)}
              </td>
              <td style={td}>{b.profitFactor >= 999 ? "∞" : b.profitFactor}</td>
              <td style={{ ...td, color: b.expectancy >= 0 ? C.green : C.red }}>
                {num(b.expectancy)}
              </td>
              <td style={{ ...td, color: b.netPnl >= 0 ? C.green : C.red, fontWeight: 700 }}>
                {num(b.netPnl)}
              </td>
...
function FilterBar(props: {
  sigFilter: "ALL" | "BUY" | "SELL" | "WAIT";
  setSigFilter: (v: "ALL" | "BUY" | "SELL" | "WAIT") => void;
  nakFilter: string;
  setNakFilter: (v: string) => void;
  nakOptions: string[];
  signFilter: string;
  setSignFilter: (v: string) => void;
  signOptions: string[];
  retroFilter: string;
  setRetroFilter: (v: string) => void;
  dowFilter: string;
  setDowFilter: (v: string) => void;
  dowOptions: string[];
  monthFilter: string;
  ...
  return (
    <section style={{ ...panel, marginTop: 14 }}>
      ...
      >
        <LabeledSelect
          label="Signal"
          value={props.sigFilter}
          onChange={(v) => props.setSigFilter(v as "ALL" | "BUY" | "SELL" | "WAIT")}
          options={["ALL", "BUY", "SELL", "WAIT"]}
        />
        <LabeledSelect
          label="Nakshatra"
          value={props.nakFilter}
          onChange={props.setNakFilter}
          options={["ALL", ...props.nakOptions]}
        />
        <LabeledSelect
          label="Moon Sign"
          value={props.signFilter}
          onChange={props.setSignFilter}
          options={["ALL", ...props.signOptions]}
        />
        <LabeledSelect
          label="Retrograde"
          value={props.retroFilter}
          onChange={props.setRetroFilter}
          options={["ALL", "0 Retro", "1 Retro", "2 Retro", "3+ Retro"]}
        />
        <LabeledSelect
          label="Day"
          value={props.dowFilter}
          onChange={props.setDowFilter}
...
const miniVal: React.CSSProperties = {
  fontFamily: "var(--eb-mono)",
  fontSize: 16,
  fontWeight: 700,
  marginTop: 4,
...
const smallHead: React.CSSProperties = {
  fontFamily: "var(--eb-mono)",
  fontSize: 11,
  color: "var(--eb-muted)",
  textTransform: "uppercase",
