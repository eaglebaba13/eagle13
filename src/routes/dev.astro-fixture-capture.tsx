...
function FixtureCapture() {
  ...
  if (!enabled) {
    return (
      ...
      >
        Dev-only. Enable with{" "}
        <code>localStorage.setItem(&quot;eb-diagnostics&quot;,&quot;on&quot;)</code>.
      </div>
    );
  }
  return <Editor />;
...
function Editor() {
  ...
  const parsed: ParseResult = useMemo(() => {
    if (!paste.trim())
      return { source: "AUTO", rows: [], planets: [], confidence: 0, warnings: [], rawText: "" };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, padding: 24, fontSize: 13 }}>
      <header style={{ marginBottom: 16 }}>
        <h1 style={{ margin: 0, fontSize: 22, color: C.gold, letterSpacing: 1 }}>
          Astro Fixture Capture · Phase 21.0C
        </h1>
        <p style={{ margin: "6px 0 0", color: C.muted, maxWidth: 780 }}>
          Paste values from Swiss / Drik / MPanchang / Prokerala. Validated JSON is generated
          locally — nothing is written to disk from the browser. Download and add to{" "}
          <code>src/lib/__fixtures__/astro-reference/</code>.
        </p>
      </header>
      ...
      >
        ...
        <div>
          <label style={label}>Timestamp (IST, local)</label>
          <input
            style={input}
            type="datetime-local"
            value={timestampIst}
            onChange={(e) => setTimestampIst(e.target.value)}
          />
        </div>
        <div>
          <label style={label}>Timestamp (UTC, derived)</label>
          <input style={input} value={utcIso} readOnly />
        </div>
        <div>
          <label style={label}>Ayanamsha mode</label>
          <input
            style={input}
            value={ayanamshaMode}
            onChange={(e) => setAyanamshaMode(e.target.value)}
          />
        </div>
        <div>
          <label style={label}>Ayanamsha (deg)</label>
          <input style={input} value={ayanamsha} onChange={(e) => setAyanamsha(e.target.value)} />
        </div>
        <div>
          <label style={label}>Node</label>
          <select
            style={input}
            value={nodeMode}
            onChange={(e) => setNodeMode(e.target.value as "mean" | "true")}
          >
        ...
        <div>
          <label style={label}>Moon</label>
          <select
            style={input}
            value={moonConvention}
            onChange={(e) => setMoonConvention(e.target.value as "geocentric" | "topocentric")}
          >
        ...
        <div>
          <label style={label}>Original-source evidence</label>
          <select
            style={input}
            value={evidence}
            onChange={(e) => setEvidence(e.target.value as OriginalSourceEvidence)}
          >
            {EVIDENCE.map((e) => (
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          <label style={label}>Notes / evidence citation</label>
          <input
            style={input}
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="e.g. Swiss Ephemeris 2.10 (Lahiri, mean node) — page/screenshot reference"
          />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ color: C.muted, fontSize: 12, marginTop: 4 }}>
          ...
          <strong
            style={{
              color: parsed.confidence >= 0.9 ? C.green : parsed.confidence >= 0.5 ? C.gold : C.red,
            }}
          >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button
              ...
              onClick={() =>
                downloadBlob(json, `${fixture.fixtureVersion}.json`, "application/json")
              }
              style={{
                background: validation.ok ? C.gold : C.border,
                color: C.bg,
                border: 0,
                padding: "6px 12px",
                borderRadius: 4,
          ...
          <p style={{ color: C.muted, fontSize: 12, marginTop: 8 }}>
            ...
            and reload{" "}
            <a href="/dev/astro-audit" style={{ color: C.gold }}>
              /dev/astro-audit
            </a>
            .
          </p>
