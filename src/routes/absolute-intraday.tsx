...
import { getGannIntradaySnapshot, type IntradaySnapshot } from "@/lib/gann-intraday.functions";
...
export const Route = createFileRoute("/absolute-intraday")({
  loader: ({ context }) => context.queryClient.ensureQueryData(intradayQuery("NIFTY50")),
  ...
  notFoundComponent: () => <div style={{ padding: 32, color: C.muted }}>Not found.</div>,
});
...
function Card({ title, value, sub }: { title: string; value: string | number; sub?: string }) {
  return (
    ...
    >
      <div
        style={{ fontSize: 10.5, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5 }}
      >
        {title}
      </div>
      <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4, fontFamily: "var(--eb-mono)" }}>
        {value}
      </div>
...
function AbsoluteIntradayPage() {
  ...
  const validation = useQuery({
    ...
    enabled: data.status === "LOCKED" || data.status === "HISTORICAL_LOCKED",
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, padding: "18px 14px 60px" }}>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        >
          VALIDATION MODE — no push alerts, no broker orders, no Decision Engine wiring. Preview
          only.
        </div>
    ...
    <>
      <div className="abs-grid" style={{ marginBottom: 14 }}>
        ...
        <Card
          ...
          value={data.previousClose.toLocaleString()}
          sub={`As of ${data.previousCloseDate}`}
        />
        ...
        <Card
          title="Raw Levels"
          value={data.rawLevels.length}
          sub={`${data.clusters.length} clusters`}
        />
        <Card
          ...
          sub={
            data.nearestSafeBuy
              ? `${data.nearestSafeBuy.planet} ${data.nearestSafeBuy.sourceLevel}`
              : ""
          }
        />
        <Card
          ...
          sub={
            data.nearestSafeSell
              ? `${data.nearestSafeSell.planet} ${data.nearestSafeSell.sourceLevel}`
              : ""
          }
        />
        <Card
        ...
        <Card
          ...
          sub={
            data.nextSafeSell ? `${data.nextSafeSell.planet} ${data.nextSafeSell.sourceLevel}` : ""
          }
        />
      </div>
      ...
      >
        {GANN_PART1_BEAR_STARS.map((s) => (
          <div key={s.nakshatra}>
            <strong style={{ color: C.text }}>{s.nakshatra}</strong> · {s.classification} · source:{" "}
            {s.evidenceSource} · observations: {s.historicalObservationCount}, monthly tops:{" "}
            {s.monthlyTops} · confidence: {s.confidence}
          </div>
        ))}
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: C.muted }}>
        Snapshot generated {new Date(data.generatedAt).toISOString()} · not connected to live
        BUY/SELL alerts, Decision Engine, or broker.
      </div>
    </>
  );
}
            ...
            {data.rankedLevels.map((l, i) => (
              <tr key={`${l.planet}-${l.sourceLevel}-${i}`}>
                ...
                <td
                  style={{
                    color:
                      l.side === "RESISTANCE" ? C.red : l.side === "SUPPORT" ? C.green : C.muted,
                  }}
                >
                  {l.side}
                </td>
...
function ExecutionTab({
  ...
  if (error) return <LoadingBox>Validation unavailable: {error.message}</LoadingBox>;
            ...
            {s.perLevel.map((p, i) => (
              <tr key={`${p.level.planet}-${p.level.sourceLevel}-${i}`}>
                ...
                <td
                  style={{
                    color:
                      p.level.side === "RESISTANCE"
                        ? C.red
                        : p.level.side === "SUPPORT"
                          ? C.green
                          : C.muted,
                  }}
                >
                ...
                <td style={{ color: p.level.safety === "SAFE" ? C.green : C.gold }}>
                  {p.level.safety}
                </td>
                ...
                <td
                  style={{
                    color:
                      p.outcome === "TARGET" ? C.green : p.outcome === "STOP" ? C.red : C.muted,
                  }}
                >
                  {p.outcome}
                </td>
        ...
        <tbody>
          {validation.simulation.perLevel.map((p, i) => (
            <tr key={`${p.level.planet}-${i}`}>
              ...
              <td style={{ color: p.cube.mandatoryPassed ? C.green : C.red }}>
                {p.cube.mandatoryPassed ? "PASS" : "FAIL"}
              </td>
              <td>
                {p.cube.conditionsAligned}/{p.cube.conditionsAvailable}
              </td>
...
function ReplayTabInner({ validation }: { validation: ValidationResult }) {
  ...
  return (
    <>
      ...
      >
        <button className="abs-tab" onClick={() => setState(restartReplay(state))}>
          ⏮ Restart
        </button>
        <button
          className="abs-tab"
          onClick={() => setState(stepReplay(state, -1))}
          disabled={cursor === 0}
        >
          ◀ Step
        </button>
        ...
        <button className="abs-tab" onClick={() => setState(jumpReplay(state, total))}>
          End ⏭
        </button>
  ...
  return (
    ...
    >
      ...
      <div style={{ color: C.muted }}>
        <strong>Inputs:</strong> previous completed trading-session close · upcoming trading day
        09:15 IST planet snapshot · absolute sidereal longitude 0°–360°.
      </div>
      <div style={{ color: C.muted, marginTop: 6 }}>
        <strong>Cycles:</strong> lower = floor(close/360)×360 · upper = ceil(close/360)×360.
      </div>
      <div style={{ color: C.muted, marginTop: 6 }}>
        <strong>Levels:</strong> L1=round(upper+deg), L2=round(lower+deg), L3=round(upper−deg),
        L4=round(lower−deg).
      </div>
      <div style={{ color: C.muted, marginTop: 6 }}>
      ...
      <div style={{ color: C.muted, marginTop: 6 }}>
        ...
        <ul style={{ margin: "4px 0 0 18px" }}>
          <li>
            {PROVISIONAL_POLICIES.EXACT_BOUNDARY}: exact 360-boundary uses previousClose + 360.
          </li>
          <li>
            {PROVISIONAL_POLICIES.CLUSTER}: cluster tolerance (5 pts NIFTY / 10 pts BANKNIFTY).
          </li>
