...
export const Route = createFileRoute("/astro")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Astro Levels | NIFTY Planetary Dashboard" },
      {
        ...
        content: "Daily planetary positions and NIFTY astro levels with a live signal engine.",
      },
  ...
  errorComponent: ({ error }) => (
    <div style={{ background: C.bg, minHeight: "100vh", padding: 40, color: C.red }}>
      <p style={{ fontFamily: "var(--eb-mono)" }}>Astro data unavailable: {error.message}</p>
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        <Card>
          ...
          <div style={{ fontSize: 22, fontWeight: 700, marginTop: 4 }}>🌙 {moon.phaseName}</div>
        ...
        <Card
          style={{
            ...
            background: `linear-gradient(135deg, rgba(37,99,235,0.12), ${C.card})`,
          }}
        >
          <div
            style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 1 }}
          ...
          <div
            style={{ fontSize: 24, fontWeight: 800, color: C.blue, marginTop: 4 }}
        ...
        <Card
          style={{
            border: `1px solid ${C.border}`,
            background: `linear-gradient(135deg, rgba(245,158,11,0.12), ${C.card})`,
          }}
        >
          <div
            style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 1 }}
          ...
          <div
            style={{ fontSize: 24, fontWeight: 800, color: C.orange, marginTop: 4 }}
            suppressHydrationWarning
          >
...
function Stat({
  ...
  return (
    <Card style={{ padding: 14 }}>
      ...
      <div
        style={{ fontSize: 22, fontWeight: 700, color: color ?? C.text, marginTop: 4 }}
        suppressHydrationWarning
      >
        {value}
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <span className="astro-mono" style={{ fontSize: 13, color: C.muted }}>
              IST {clock}
            </span>
            <span
              style={{
        ...
        >
          ...
          <main style={{ flex: 1, minWidth: 0 }}>
            {isFetching ? (
              <div
                className="no-print astro-mono"
                style={{ fontSize: 11, color: C.orange, marginBottom: 8 }}
              >
            ...
            >
              ...
              <Stat
                label="Live NIFTY"
                value={<span className="astro-mono">{data.livePrice.toLocaleString("en-IN")}</span>}
                color={C.blue}
                sub={`Prev close ${data.prevClose.toLocaleString("en-IN")}`}
              />
              <Stat
                label="Nearest Level"
              ...
              <Stat
                ...
                color={
                  signal.confidence >= 60 ? C.green : signal.confidence <= 40 ? C.red : C.orange
                }
            ...
            >
              ...
              <Stat
                label="Moon Degree"
                value={<span className="astro-mono">{data.moonDegree.toFixed(2)}°</span>}
              />
              <Stat
                label="Retrograde"
              ...
              <Stat
                label="Day EMA 13"
                value={
                  <span className="astro-mono">{data.ema13 != null ? num(data.ema13) : "—"}</span>
                }
                color={
                  data.emaBias === "Bullish" ? C.green : data.emaBias === "Bearish" ? C.red : C.text
                }
                sub={
                  data.emaBias
                    ? `${data.emaBias} · price ${data.emaBias === "Bullish" ? "above" : "below"} EMA 13`
                    : "day timeframe"
                }
              />
              <Stat
              ...
              <Stat
                label="Ayanamsa"
                value={<span className="astro-mono">{data.ayanamsa.toFixed(3)}°</span>}
              />
              <Stat
                label="Prev Close"
                value={<span className="astro-mono">{data.prevClose.toLocaleString("en-IN")}</span>}
                sub={data.prevDate}
              />
            </div>
            ...
            <div id="levels" style={{ scrollMarginTop: 90, marginBottom: 16 }}>
              ...
              >
                ...
                <span style={{ fontSize: 11, color: C.muted }}>
                  Sorted by distance to live price
                </span>
              </div>
            ...
            >
              ...
              <button onClick={() => window.print()} style={btn(C.orange)}>
                Print
              </button>
              <button onClick={() => refetch()} style={btn(C.muted)}>
                Refresh
              </button>
            </div>
            ...
            <div id="planets" style={{ scrollMarginTop: 90, marginBottom: 16 }}>
              <div style={{ padding: "0 2px 10px", fontWeight: 700, letterSpacing: 1 }}>
                PLANETARY POSITIONS
              </div>
              <div
                className="astro-grid"
            ...
            <Card style={{ marginBottom: 16, padding: 0, overflow: "hidden" }}>
              ...
              >
                <span style={{ fontWeight: 700, letterSpacing: 1 }}>
                  ASTRO LEVELS (R1 / S1 / R2 / S2)
                </span>
                <span className="astro-mono" style={{ fontSize: 11, color: C.muted }}>
                  Base {data.cycles.base} · Upper {num(data.cycles.upper)} · Lower{" "}
                  {num(data.cycles.lower)}
                </span>
              </div>
                  ...
                  <tbody>
                    {filtered.map((p) => (
                      <tr key={p.planet}>
                        ...
                        <td className="astro-mono" style={{ color: C.red }}>
                          {num(p.r1)}
                        </td>
                        <td className="astro-mono" style={{ color: C.green }}>
                          {num(p.s1)}
                        </td>
                        <td className="astro-mono" style={{ color: C.red }}>
                          {num(p.r2)}
...
function PlanetCard({ p }: { p: PlanetRow }) {
  ...
  return (
    <div className="eb-card eb-glass eb-planet-card" style={orbStyleFor(p.planet)}>
      ...
      <div
        style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px", fontSize: 12.5 }}
      >
        <Field
          label="Nakshatra"
          value={<NakBadge bull={p.bull} bear={p.bear} name={p.nakshatra} />}
        />
      ...
      <div>
        ...
        >
          ...
          <span className="astro-mono">{strength}%</span>
        </div>
        ...
        return (
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {i === 0 ? (
                <span style={{ fontSize: 9, fontWeight: 700, color: C.blue, letterSpacing: 1 }}>
                  NEAREST
                </span>
              ) : null}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
              <span style={{ color: C.muted }}>
                Dist{" "}
                <b className="astro-mono" style={{ color: C.text }}>
      ...
      <div className="eb-card eb-glass eb-anim-border" style={{ borderRadius: 18, padding: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
          <div
            ...
            style={{ width: 46, height: 46, ...orbStyleFor("Moon") }}
          />
          <div>
      ...
      <div className="eb-card eb-glass" style={{ borderRadius: 18, padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-around", textAlign: "center" }}>
          <div>
            ...
            <div className="astro-mono" style={{ fontWeight: 700 }} suppressHydrationWarning>
              {sun.sunrise}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {retro.map((p) => (
              ...
              >
                <span
                  className="eb-planet-orb"
                  style={{ width: 14, height: 14, ...orbStyleFor(p.planet) }}
                />
                {p.planet}
              </span>
            ))}
...
function AstroCharts({
  ...
  return (
    <div
      className="astro-grid"
      style={{ gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", marginBottom: 16 }}
    >
      <Card>
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1, color: C.muted }}>
          SIGNAL CONFIDENCE
        </div>
        <ApexChart
          type="radialBar"
      ...
      <Card>
        ...
        <ApexChart
          ...
          height={240}
          series={[
            { name: "Speed", data: speedPlanets.map((p) => Number(Math.abs(p.speed).toFixed(3))) },
          ]}
          options={{
            colors: ["#d4af37"],
            xaxis: {
