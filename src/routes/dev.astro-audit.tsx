    ...
    >
      <header style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: "6px 0 0", color: C.muted, maxWidth: 780 }}>
          Read-only astronomical precision audit. Compares EagleBaba's current engine against
          captured Swiss / Drik / MPanchang / Prokerala reference fixtures. Production astronomy is{" "}
          <strong>not</strong> modified by this page.
        </p>
        <div
          style={{
            marginTop: 10,
        ...
        >
          <strong style={{ color: C.gold }}>PROVISIONAL METHODOLOGY DEFAULT</strong> — Mean Node ·
          Geocentric Moon · Lahiri (Chitrapaksha). Preserves current output and matches Drik
          Panchang default. Not a proven original Gann convention. Evidence tiers: VERIFIED_FACT ·
          DOCUMENTED_DEFAULT · INFERENCE · HYPOTHESIS · BACKTEST_RESULT.
        </div>
      </header>

      {q.isLoading && <p style={{ color: C.muted }}>Running audit…</p>}
      {q.error && <p style={{ color: C.red }}>Audit failed: {(q.error as Error).message}</p>}
      ...
      {q.data && q.data.fixtures === 0 && (
        ...
        >
          ...
          <p style={{ color: C.muted, margin: "6px 0 0" }}>
            Add captured JSON fixtures to <code>src/lib/__fixtures__/astro-reference/</code>. See
            the README in that folder for capture rules. Schema example is at{" "}
          ...
          >
            <div>
              <div style={{ color: C.gold, fontWeight: 600 }}>
                ...
                <span style={{ fontFamily: "monospace", fontSize: 12 }}>{r.mode}</span>
              </div>
              <div style={{ color: C.muted, fontSize: 12 }}>
                {r.fixture.timestampIso} · {r.fixture.location.label} · {r.fixture.referenceEngine}
              </div>
              <div style={{ color: C.muted, fontSize: 12 }}>
                Ayanamsha {r.fixture.ayanamshaMode} · Node {r.fixture.nodeMode} · Moon{" "}
                {r.fixture.moonConvention}
              </div>
            </div>
            <button
          ...
          >
            ...
            <div>
              ...
              <span style={{ color: statusColor(r.ayanamsha.toleranceStatus) }}>
                {r.ayanamsha.diffDeg.toFixed(6)}° ({r.ayanamsha.diffArcsec.toFixed(1)}″) ·{" "}
                {r.ayanamsha.toleranceStatus}
              </span>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 8, color: C.muted, fontSize: 12 }}>
        Fixtures:{" "}
        <strong style={{ color: cov.meetsMinimum ? C.green : C.red }}>
      ...
      {!cov.meetsMinimum && (
        ...
        >
          Stop condition: fewer than {MIN_FIXTURES_FOR_PRODUCTION_VERDICT} valid fixtures. Final
          methodology verdict remains{" "}
          <strong style={{ color: C.red }}>CANNOT_DETERMINE_WITHOUT_ORIGINAL_SOURCE</strong>. Add
          fixtures via{" "}
          <a style={{ color: C.gold }} href="/dev/astro-fixture-capture">
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
        <button
          style={btn}
          onClick={() =>
            downloadBlob(planetComparisonCsv(reports), "planet-comparison.csv", "text/csv")
          }
        >
          Planet CSV
        </button>
        <button
          style={btn}
          onClick={() => downloadBlob(boundaryRiskCsv(risks), "boundary-risks.csv", "text/csv")}
        >
          Boundary CSV
        </button>
        <button
          ...
          onClick={() =>
            downloadBlob(
              JSON.stringify({ coverage: cov, modes, risks }, null, 2),
      ...
      {modes.map((m) => (
        ...
        >
          <div style={{ color: C.gold, fontWeight: 600 }}>
            {m.mode}{" "}
            <span style={{ color: C.muted, fontWeight: 400 }}>
              · {m.fixtures} fixture(s) · levels changed in {m.levelsChangedFixtures} · max Δ{" "}
              {m.maxLevelDelta}
            </span>
          </div>
            ...
            <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ color: C.muted, textAlign: "left" }}>
                  <th style={th}>Planet</th>
                  <th style={th}>n</th>
                  <th style={th}>|Δ| mean</th>
                  <th style={th}>|Δ| max</th>
              ...
              <tbody>
                {[...(m.moon ? [m.moon] : []), ...m.perPlanet].map((p) => (
                  <tr key={p.planet} style={{ borderTop: `1px solid ${C.border}` }}>
                    ...
                    <td style={td}>{p.exactPct}</td>
                    <td style={td}>{p.acceptablePct}</td>
                    <td style={td}>{p.warningPct}</td>
                    <td style={td}>{p.failPct}</td>
                    <td style={td}>{p.signMatchPct}</td>
                    <td style={td}>{p.nakshatraMatchPct}</td>
                    <td style={td}>{p.padaMatchPct}</td>
                    <td style={td}>{p.retroMatchPct}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {risks.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <div style={{ color: C.gold, fontWeight: 600, marginBottom: 6 }}>
            Boundary Risks ({risks.length})
          </div>
          <div
            style={{
              maxHeight: 240,
