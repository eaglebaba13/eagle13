...
export const Route = createFileRoute("/market-replay")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Replay any past intraday session with the EagleBABA astro signal engine." },
...
function MarketReplayPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const p = JSON.parse(raw) as Partial<{
        symbol: ReplaySymbol; date: string; timeframe: Timeframe; speed: number;
        entryMode: EntryMode; policy: AmbiguousPolicy; slippagePct: number;
        showLevels: boolean; showSignals: boolean;
      }>;
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(PRESET_KEY, JSON.stringify({
        symbol, date, timeframe, speed, entryMode, policy, slippagePct, showLevels, showSignals,
      }));
    } catch { /* ignore quota */ }
  }, [hydrated, symbol, date, timeframe, speed, entryMode, policy, slippagePct, showLevels, showSignals]);

  const load = useCallback(async () => {
    setLoading(true); setError(null);
    setSession(null); setIdx(0); setTrades([]); setEvents([]);
    ...
    try {
      ...
      if (s.candles.length === 0) {
        setError("No candles returned for this session. Try a more recent date or different timeframe.");
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    }, periodMs, { immediate: false, name: `replay-${speed}x` });
    return unsub;
      ...
      if ((sig.signal === "BUY" || sig.signal === "SELL") && activeIdRef.current == null) {
        ...
        setTrades((ts) => [
          ...ts,
          {
            ...
            status: {
              status: "PENDING", entry: null, entryIndex: null, exit: null, exitIndex: null,
              mfe: 0, mae: 0, ambiguous: false, grossPnl: 0, netPnl: 0, pnlPct: 0, costs: 0,
            },
          },
        ]);
      }
    ...
    setTrades((ts) => {
      ...
      if (closed && activeIdRef.current === activeId) {
        ...
        setEvents((e) => [
          ...e,
          {
            ts: session.candles[Math.min(status.exitIndex ?? tick.currentIndex, session.candles.length - 1)].ts,
            index: status.exitIndex ?? tick.currentIndex,
            type: status.ambiguous ? "AMBIGUOUS" : status.status === "TARGET_HIT" ? "TARGET_HIT" : "STOP_HIT",
  ...
  const onStepFwd = () => { setIdx((i) => Math.min(total - 1, i + 1)); };
  const onStepBack = () => { setIdx((i) => Math.max(0, i - 1)); };
  const onRestart = () => { setIdx(0); setPlaying(false); resetTradingState(); };
  const resetTradingState = () => {
    setTrades([]); setEvents([]);
    activeIdRef.current = null; tradeIdRef.current = 0;
    lastSignalRef.current = "WAIT";
    signalCountsRef.current = { buy: 0, sell: 0, wait: 0 };
  };
  const onPrevSession = () => setDate((d) => daysAgoIso(daysBetween(d, todayIso()) + 1));
  const onNextSession = () => setDate((d) => {
  ...
  const cfg: ReplayConfig | null = session ? {
    symbol: session.symbol, date: session.date, timeframe: session.timeframe,
    provider: session.provider, entryMode, policy,
    costs: { slippagePct, brokerageFlat: 0, brokeragePct: 0 },
  } : null;
  const runId = cfg ? computeReplayRunId(cfg) : "—";

  const closedTrades: ClosedTrade[] = trades
    .filter((t) => t.status.status === "TARGET_HIT" || t.status.status === "STOP_HIT" || t.status.status === "EXITED")
    .map((t) => ({
      signal: t.signal,
  ...
  const exportCsv = () => {
    if (!session) return;
    const head = ["id","signal","signalTs","entry","target","stop","exit","status","ambiguous","mfe","mae","pnl","pnlPct","confidence","moonSign","moonNakshatra","planet"];
    const rows = trades.map((t) => [
      t.id, t.signal, new Date(t.signalTs).toISOString(),
      t.status.entry ?? "", t.target ?? "", t.stop ?? "", t.status.exit ?? "",
      t.status.status, t.status.ambiguous, t.status.mfe, t.status.mae,
      t.status.netPnl, t.status.pnlPct, t.confidence,
      t.moonSign, t.moonNakshatra, t.planet ?? "",
    ]);
    const csv = [head, ...rows].map((r) => r.map(csvCell).join(",")).join("\n");
    downloadBlob(csv, `replay-${session.symbol}-${session.date}-${session.timeframe}.csv`, "text/csv");
  };
  const exportJson = () => {
    if (!session || !cfg) return;
    const payload = { runId, config: cfg, session: { ...session, candles: session.candles.length }, trades, events, stats };
    downloadBlob(JSON.stringify(payload, null, 2), `replay-${session.symbol}-${session.date}-${session.timeframe}.json`, "application/json");
  };
  ...
  const chart = useMemo(() => {
    ...
    const yPoints: { y: number; borderColor: string; label: { text: string; style: { color: string; background: string } } }[] = [];
    if (showLevels) {
      // Use nearest planet's R1/R2/R3/S1/S2/S3 for context.
      const planets = session.planets;
      // pick planet closest to entry price
      const price = tick.price;
      const nearest = [...planets].sort((a, b) => Math.abs(a.r1 - price) - Math.abs(b.r1 - price))[0];
      if (nearest) {
        const defs: [string, number, string][] = [
          ["R1", nearest.r1, C.red], ["R2", nearest.r2, C.red], ["R3", nearest.r3, C.red],
          ["S1", nearest.s1, C.green], ["S2", nearest.s2, C.green], ["S3", nearest.s3, C.green],
        ];
    ...
    if (showSignals) {
      ...
      if (active) {
        if (active.target != null) yPoints.push({
          y: active.target, borderColor: C.orange,
          label: { text: `Target ${fmtNum(active.target)}`, style: { color: "#04140b", background: C.orange } },
        });
        if (active.stop != null) yPoints.push({
          y: active.stop, borderColor: C.blue,
          label: { text: `Stop ${fmtNum(active.stop)}`, style: { color: "#04140b", background: C.blue } },
        });
      }
    }
  ...
  const sessionProgress = session && session.candles.length > 0
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, padding: "18px 16px 96px" }}>
      ...
      <section style={panel}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
          <Field label="Instrument">
            <select value={symbol} onChange={(e) => setSymbol(e.target.value as ReplaySymbol)} style={selectStyle}>
              {(Object.keys(REPLAY_SYMBOLS) as ReplaySymbol[]).map((k) => (
                <option key={k} value={k}>{REPLAY_SYMBOLS[k].label}</option>
              ))}
            </select>
          </Field>
          ...
          <Field label="Timeframe">
            <select value={timeframe} onChange={(e) => setTimeframe(e.target.value as Timeframe)} style={selectStyle}>
              {TIMEFRAMES.map((tf) => (
                <option key={tf} value={tf}>{tf} · max {YAHOO_TIMEFRAME_LIMITS[tf].maxAgeDays}d</option>
              ))}
            </select>
          </Field>
          <Field label="Entry Mode">
            <select value={entryMode} onChange={(e) => setEntryMode(e.target.value as EntryMode)} style={selectStyle}>
          ...
          <Field label="Ambiguous Policy">
            <select value={policy} onChange={(e) => setPolicy(e.target.value as AmbiguousPolicy)} style={selectStyle}>
          ...
          <Field label="Slippage %">
            <input type="number" step="0.01" value={slippagePct}
              onChange={(e) => setSlippagePct(Number(e.target.value) || 0)}
              style={selectStyle} />
          </Field>
          <div style={{ display: "flex", alignItems: "flex-end" }}>
            <button onClick={load} disabled={loading} style={{ ...btnPrimary, width: "100%", cursor: loading ? "wait" : "pointer", opacity: loading ? 0.6 : 1 }}>
              {loading ? "Loading…" : "▶ Load Session"}
            </button>
          </div>
        </div>
        {error ? <div style={{ marginTop: 10, color: C.red, fontFamily: "var(--eb-mono)", fontSize: 12 }}>{error}</div> : null}
      </section>
      ...
      {session ? (
        <section style={{ ...panel, marginTop: 14 }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            <button style={btnGhost} onClick={onPrevSession}>« Prev Session</button>
            <button style={btnGhost} onClick={onStepBack} disabled={!canStep || idx === 0}>◀ Step</button>
            {playing ? (
              <button style={btnPrimary} onClick={() => setPlaying(false)}>❚❚ Pause</button>
            ) : (
              <button style={btnPrimary} onClick={() => setPlaying(true)} disabled={!canStep}>▶ Play</button>
            )}
            <button style={btnGhost} onClick={onStepFwd} disabled={!canStep || idx >= total - 1}>Step ▶</button>
            <button style={btnGhost} onClick={onRestart}>⟲ Restart</button>
            <button style={btnGhost} onClick={onNextSession}>Next Session »</button>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: "auto" }}>
              <span style={fieldLbl}>Speed</span>
              {SPEEDS.map((s) => (
                <button key={s} onClick={() => setSpeed(s)}
                  style={{ ...chip, background: speed === s ? C.orange : "transparent", color: speed === s ? "#04140b" : C.text }}>
                  {s}×
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 10, alignItems: "center", fontFamily: "var(--eb-mono)", fontSize: 12 }}>
            <span style={{ color: C.muted }}>Session:</span>
            <span>{session.label} · {session.date}</span>
            <span style={{ color: C.muted }}>Clock:</span>
            <span style={{ color: C.orange }}>{session.candles[idx] ? fmtClock(session.candles[idx].ts, session.timezone) : "—"} {session.timezone === "UTC" ? "UTC" : "IST"}</span>
          ...
          <input
            type="range" min={0} max={Math.max(0, total - 1)} value={Math.min(idx, Math.max(0, total - 1))}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
            <label style={toggleLbl}>
              <input type="checkbox" checked={showLevels} onChange={(e) => setShowLevels(e.target.checked)} /> Show Levels
            </label>
            <label style={toggleLbl}>
              <input type="checkbox" checked={showSignals} onChange={(e) => setShowSignals(e.target.checked)} /> Show Target/Stop
            </label>
      ...
      {/* Chart + side panel */}
      {session && chart && tick ? (
        <section style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 320px", gap: 12, marginTop: 14 }}>
          <div style={{ ...panel, minWidth: 0 }}>
            <ApexChart
              ...
              series={chart.series}
              options={{
                chart: { toolbar: { show: true }, zoom: { enabled: true }, animations: { enabled: false } },
                xaxis: { type: "datetime", labels: { style: { colors: "var(--eb-muted)" } } },
                yaxis: { tooltip: { enabled: true }, labels: { style: { colors: "var(--eb-muted)" } } },
                annotations: { yaxis: chart.yPoints },
                grid: { borderColor: "var(--eb-border)" },
                tooltip: { theme: "dark" },
                plotOptions: { candlestick: { colors: { upward: "var(--eb-bull)", downward: "var(--eb-bear)" } } },
              }}
            />
          </div>
          <SidePanel session={session} tick={tick} activeTrade={trades.find((t) => t.id === activeIdRef.current) ?? null} />
        </section>
      ) : null}
      ...
      {session ? (
        <section style={{ ...panel, marginTop: 14 }}>
          <SectionHead>Trade Log · {trades.length} signal{trades.length === 1 ? "" : "s"}</SectionHead>
          {trades.length === 0 ? (
            <div style={{ color: C.muted, fontFamily: "var(--eb-mono)", fontSize: 12 }}>No trades yet — press Play or step through candles.</div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={tableStyle}>
                <thead>
                  <tr>
                    {["#","Time","Signal","Entry","Target","Stop","Exit","Result","Ambig","MFE","MAE","PnL","Conf","Nakshatra","Planet"].map((h) => (
                ...
                <tbody>
                  {trades.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td style={{ ...td, color: t.signal === "BUY" ? C.green : C.red }}>{t.signal}</td>
                      ...
                      <td style={{ ...td, color: t.status.status === "TARGET_HIT" ? C.green : t.status.status === "STOP_HIT" ? C.red : C.muted }}>
                        {t.status.status}
                      </td>
                      <td style={td}>{t.status.ambiguous ? "yes" : "—"}</td>
                      <td style={td}>{fmtNum(t.status.mfe)}</td>
                      <td style={td}>{fmtNum(t.status.mae)}</td>
                      <td style={{ ...td, color: t.status.netPnl > 0 ? C.green : t.status.netPnl < 0 ? C.red : C.muted }}>
      ...
      {session ? (
        <section style={{ ...panel, marginTop: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8, fontFamily: "var(--eb-mono)", fontSize: 12 }}>
      ...
      {session && events.length > 0 ? (
        <section style={{ ...panel, marginTop: 14 }}>
          ...
          <div style={{ maxHeight: 220, overflowY: "auto", fontFamily: "var(--eb-mono)", fontSize: 12 }}>
            {events.map((ev, i) => (
              <div key={i} style={{ display: "flex", gap: 10, padding: "4px 0", borderBottom: `1px solid ${C.border}`, cursor: "pointer" }}
                onClick={() => jumpToTime(ev.ts)}>
                <span style={{ color: C.muted, minWidth: 80 }}>{fmtClock(ev.ts, session.timezone)}</span>
                <span style={{ color: ev.type === "SIGNAL" ? C.blue : ev.type === "TARGET_HIT" ? C.green : ev.type === "STOP_HIT" ? C.red : C.orange, minWidth: 110 }}>{ev.type}</span>
                <span>{ev.label}</span>
              </div>
            ))}
          </div>
      ...
      {session ? (
        <section style={{ ...panel, marginTop: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 8, fontFamily: "var(--eb-mono)", fontSize: 11 }}>
            <Kv k="Provider" v={`${session.provider} (${session.interval})`} />
            <Kv k="Timezone" v={session.timezone} />
            <Kv k="Entry Rule" v={entryMode === "next_open" ? "Next Candle Open" : "Signal Candle Close"} />
          ...
          <div style={{ marginTop: 10, color: C.muted, fontFamily: "var(--eb-mono)", fontSize: 11, lineHeight: 1.6 }}>
            <div>{session.dataQuality.limitationNote}</div>
            {session.disclaimers.map((d, i) => <div key={i}>• {d}</div>)}
          </div>
        </section>
      ) : null}
    </div>
...
function SidePanel({
  session, tick, activeTrade,
}: {
  session: ReplaySession;
  tick: { currentIndex: number; price: number; board: ReturnType<typeof buildLevelBoard>; sig: ReturnType<typeof computeSignal>; candle: Candle };
  activeTrade: OpenTrade | null;
}) {
  const nearestSup = tick.board.filter((b) => !b.isResistance && b.value <= tick.price).sort((a, b) => tick.price - a.value - (tick.price - b.value))[0];
  const nearestRes = tick.board.filter((b) => b.isResistance && b.value >= tick.price).sort((a, b) => a.value - tick.price - (b.value - tick.price))[0];
  const near = tick.sig.nearest;
  const bias =
    session.bullRetroCount > session.bearRetroCount ? "Bullish tilt" :
    session.bearRetroCount > session.bullRetroCount ? "Bearish tilt" :
    "Neutral";
  return (
    <aside style={panel}>
      ...
      <div style={{ fontFamily: "var(--eb-mono)", fontSize: 12, display: "grid", gap: 6 }}>
        ...
        <Kv k="Signal" v={`${tick.sig.emoji} ${tick.sig.signal}`} c={tick.sig.signal === "BUY" ? C.green : tick.sig.signal === "SELL" ? C.red : C.muted} />
      ...
      {activeTrade ? (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
          ...
          <div style={{ fontFamily: "var(--eb-mono)", fontSize: 12, display: "grid", gap: 6 }}>
            <Kv k="Signal" v={activeTrade.signal} c={activeTrade.signal === "BUY" ? C.green : C.red} />
            ...
            <Kv k="PnL" v={fmtNum(activeTrade.status.netPnl)} c={activeTrade.status.netPnl >= 0 ? C.green : C.red} />
          </div>
        </div>
      ) : null}
    </aside>
...
const selectStyle: React.CSSProperties = {
  width: "100%", background: "var(--eb-bg)", color: "var(--eb-text)",
...
const btnGhost: React.CSSProperties = {
  ...
  border: "1px solid var(--eb-border)", borderRadius: 6,
  padding: "6px 10px", fontFamily: "var(--eb-mono)", fontSize: 11, cursor: "pointer",
  minHeight: 36,
};
const toggleLbl: React.CSSProperties = {
  display: "inline-flex", alignItems: "center", gap: 6,
  fontFamily: "var(--eb-mono)", fontSize: 11, color: "var(--eb-muted)",
  padding: "4px 8px", border: `1px solid var(--eb-border)`, borderRadius: 6,
};
