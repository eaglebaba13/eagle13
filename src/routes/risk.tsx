...
function RiskPage() {
  ...
  if (!hydrated)
    return <div style={{ padding: 24, color: "var(--eb-muted)" }}>Loading risk workstation…</div>;
...
function RiskWorkstation() {
  ...
  const decisionBias: "BULL" | "BEAR" | "NEUTRAL" =
    decision?.action === "STRONG_BUY_CE" || decision?.action === "BUY_CE"
      ...
      : decision?.action === "STRONG_BUY_PE" || decision?.action === "BUY_PE"
        ? "BEAR"
        : "NEUTRAL";
  const optionsAgreement = !!decision?.contributions.find(
  ...
  return (
    ...
    >
      ...
      <ChecklistCard
        checklist={checklist}
        onCommit={addOpenPosition}
        disabled={!checklist.allPass || daily.stopTrading}
      />
...
function PageHeader({
  ...
  riskLevel: RiskLevel;
  daily: ReturnType<typeof dailyLimitCheck>;
}) {
    ...
    >
      <div>
        ...
        <p style={{ margin: "4px 0 0", color: "var(--eb-muted)", fontSize: 13 }}>
          Position sizing · portfolio heat · daily limits · journal — reuses the Decision Engine,
          never overrides it.
        </p>
      </div>
...
function SettingsCard({
  ...
  return (
    <section style={cardStyle}>
      <h2 style={{ marginTop: 0, fontSize: 16, display: "flex", gap: 6, alignItems: "center" }}>
        <Wallet size={16} /> Account Settings
      </h2>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
        <div>
          ...
          <select
            ...
            onChange={(e) => upd("profile", e.target.value as RiskProfile)}
          >
        ...
        <NumField
          ...
          onChange={(v) => upd("capital", v)}
        />
        <NumField
          label="Risk % per trade"
          value={settings.riskPct}
          step={0.1}
          onChange={(v) => upd("riskPct", v)}
          disabled={settings.profile !== "CUSTOM"}
        />
        <NumField
          label="Brokerage / lot (₹)"
          value={settings.brokeragePerLot}
          onChange={(v) => upd("brokeragePerLot", v)}
        />
        <NumField
          label="Slippage / unit (₹)"
          value={settings.slippagePerUnit}
          step={0.1}
          onChange={(v) => upd("slippagePerUnit", v)}
        />
        <NumField
          label="Min Risk-Reward"
          value={settings.minRR}
          step={0.1}
          onChange={(v) => upd("minRR", v)}
        />
        <NumField
          label="Max Daily Loss %"
          value={settings.maxDailyLossPct}
          step={0.1}
          onChange={(v) => upd("maxDailyLossPct", v)}
        />
        <NumField
          label="Max Trades / Day"
          value={settings.maxTradesPerDay}
          onChange={(v) => upd("maxTradesPerDay", v)}
        />
      </div>
      <div
        style={{
      ...
      >
        Profile budget · Daily {defaults.dailyRiskPct}% · Weekly {defaults.weeklyRiskPct}% · Monthly{" "}
        {defaults.monthlyRiskPct}% · Max open {defaults.maxOpenTrades} · Same-direction{" "}
        {defaults.maxSameDirection} · Min confidence {defaults.minConfidence}
      </div>
    </section>
  );
}
...
function TradeBuilder(props: {
  ...
  setSymbol: (v: "NIFTY" | "BANKNIFTY") => void;
  direction: Direction;
  ...
  supportsText: string;
  setSupportsText: (s: string) => void;
  resistancesText: string;
  setResistancesText: (s: string) => void;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
        <div>
          ...
          <select
            style={input}
            value={props.symbol}
            onChange={(e) => props.setSymbol(e.target.value as "NIFTY" | "BANKNIFTY")}
          >
        ...
        <div>
          ...
          <input
            style={input}
            value={props.supportsText}
            onChange={(e) => props.setSupportsText(e.target.value)}
          />
        </div>
        <div>
          ...
          <input
            style={input}
            value={props.resistancesText}
            onChange={(e) => props.setResistancesText(e.target.value)}
          />
        </div>
      </div>
      <div
      ...
      >
        <div style={{ fontSize: 13 }}>
          ...
          {props.stopSuggest.stop != null ? (
            <>
              SL {props.stopSuggest.stop} · Target {props.stopSuggest.target} · RR{" "}
              {props.stopSuggest.riskReward}{" "}
              <span style={{ color: "var(--eb-muted)" }}>({props.stopSuggest.note})</span>
            </>
          ) : (
        ...
        <button
          ...
          disabled={props.stopSuggest.stop == null}
        >
          Apply SL
        </button>
      </div>
...
function PositionCard({
  ...
  return (
    <section style={cardStyle}>
      ...
      {position.valid ? (
        <div
          style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8, fontSize: 14 }}
        >
      ...
      >
        <div style={{ fontSize: 13, color: "var(--eb-muted)" }}>
          Decision: <strong style={{ color: "var(--eb-text)" }}>{decision}</strong> · Confidence{" "}
          {confidence}
        </div>
        <div
          style={{
            padding: "6px 12px",
            background: gradeColor[quality.grade],
            color: "white",
            borderRadius: 8,
            fontWeight: 700,
          }}
...
function PortfolioHeatCard({
  heat,
  riskLevel,
  defaults,
}: {
  heat: ReturnType<typeof computePortfolioHeat>;
  riskLevel: RiskLevel;
  defaults: ProfileDefaults;
}) {
  ...
  return (
    <section style={cardStyle}>
      ...
      >
        ...
        <Row
          k="Exposure"
          v={`₹${Math.round(heat.exposure).toLocaleString()} (${heat.exposurePct.toFixed(1)}%)`}
        />
      ...
      <div style={{ marginTop: 10, fontSize: 12, color: "var(--eb-muted)" }}>
        Budget: Daily {defaults.dailyRiskPct}% · Weekly {defaults.weeklyRiskPct}% · Monthly{" "}
        {defaults.monthlyRiskPct}%
      </div>
      {Object.keys(heat.sectorExposure).length > 0 && (
        <div style={{ marginTop: 8, fontSize: 12 }}>
          <strong>Sectors:</strong>{" "}
          {Object.entries(heat.sectorExposure)
...
function ChecklistCard({
  ...
  checklist: ReturnType<typeof preTradeChecklist>;
  onCommit: () => void;
  disabled: boolean;
}) {
  return (
    <section style={cardStyle}>
      <h2 style={{ marginTop: 0, fontSize: 16, display: "flex", alignItems: "center", gap: 6 }}>
        <ListChecks size={16} /> Pre-Trade Checklist
      </h2>
      <ul
        style={{
          listStyle: "none",
      ...
      >
        {checklist.items.map((it) => (
          ...
          >
            <span
              style={{
                color: it.pass ? "var(--eb-bull)" : "var(--eb-bear)",
                fontWeight: 700,
                fontSize: 16,
              }}
            >
            ...
            <div>
              ...
              {it.detail && (
                <div style={{ fontSize: 12, color: "var(--eb-muted)" }}>{it.detail}</div>
              )}
            </div>
          </li>
        ))}
...
function OpenPositionsCard({
  ...
  onClose: (id: string, outcome: "WIN" | "LOSS" | "BREAKEVEN", pnl: number) => void;
}) {
  return (
    <section style={cardStyle}>
              ...
              <tr key={p.id} style={{ borderTop: "1px solid var(--eb-border)" }}>
                ...
                <td style={{ display: "flex", gap: 4, padding: "6px 0" }}>
                  <button
                    ...
                    onClick={() => onClose(p.id, "WIN", p.riskAmount * 1.5)}
                  >
                    Win
                  </button>
                  <button
                    style={{ ...btnGhost, padding: "4px 8px" }}
                    onClick={() => onClose(p.id, "LOSS", -p.riskAmount)}
                  >
                    Loss
                  </button>
                  <button
                    style={{ ...btnGhost, padding: "4px 8px" }}
                    onClick={() => onClose(p.id, "BREAKEVEN", 0)}
                  >
                    BE
                  </button>
                </td>
...
function DailyLimitsCard({
  ...
  stats: { trades: number; wins: number; losses: number; pnl: number; riskUsed: number };
  daily: ReturnType<typeof dailyLimitCheck>;
  settings: Settings;
  defaults: ProfileDefaults;
}) {
  return (
    <section style={cardStyle}>
      <h2 style={{ marginTop: 0, fontSize: 16, display: "flex", alignItems: "center", gap: 6 }}>
        <TrendingDown size={16} /> Daily Tracker
      </h2>
      <div
        style={{
          display: "grid",
      ...
      >
        ...
        <Metric
          k="PnL"
          v={`₹${Math.round(stats.pnl).toLocaleString()}`}
          bad={stats.pnl < 0}
          good={stats.pnl > 0}
        />
    ...
    >
      ...
      <div
        style={{
          fontSize: 16,
          fontWeight: 700,
          color: good ? "var(--eb-bull)" : bad ? "var(--eb-bear)" : "var(--eb-text)",
        }}
      >
        {v}
      </div>
  ...
  return (
    <section style={cardStyle}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ marginTop: 0, fontSize: 16, display: "flex", alignItems: "center", gap: 6 }}>
          <BookOpen size={16} /> Trade Journal ({journal.length})
        </h2>
        {journal.length > 0 && (
          <button
            style={btnGhost}
            onClick={() => {
              if (confirm("Clear all journal entries?")) onClear();
            }}
          >
      ...
      >
        <div>
          <label style={label}>Symbol</label>
          <input style={input} value={symbol} onChange={(e) => setSymbol(e.target.value)} />
        </div>
        <div>
          <label style={label}>Direction</label>
      ...
      >
        <div>
          <label style={label}>Reason / decision</label>
          <input
            style={input}
            value={reason}
            onChange={(e) => setReason(e.target.value)}
            placeholder="e.g. Astro BULL confluence"
          />
        </div>
        <div>
          <label style={label}>Lessons</label>
          <input
            style={input}
            value={lessons}
            ...
            <tbody>
              {journal.slice(0, 50).map((e) => (
                <tr key={e.id} style={{ borderTop: "1px solid var(--eb-border)" }}>
                  ...
                  <td>{e.entry}</td>
                  <td>{e.exit ?? "—"}</td>
                  <td style={{ color: (e.pnl ?? 0) >= 0 ? "var(--eb-bull)" : "var(--eb-bear)" }}>
                    ₹{Math.round(e.pnl ?? 0).toLocaleString()}
                  </td>
                  <td>{e.outcome}</td>
                  <td>{e.reason}</td>
                </tr>
...
function ReportsCard({
  ...
  daily: ReturnType<typeof summariseJournal>;
  weekly: ReturnType<typeof summariseJournal>;
  monthly: ReturnType<typeof summariseJournal>;
}) {
  const cols: { title: string; r: ReturnType<typeof summariseJournal> }[] = [
    { title: "Daily", r: daily },
    { title: "Weekly", r: weekly },
    { title: "Monthly", r: monthly },
  ];
  return (
    <section style={cardStyle}>
      ...
      >
        {cols.map((c) => (
          ...
          >
            ...
            <Row
              k="Profit factor"
              v={Number.isFinite(c.r.profitFactor) ? c.r.profitFactor.toFixed(2) : "∞"}
            />
          </div>
        ))}
      </div>
